...
#-------------------------------------
NDK_VER=`echo "$NDK_ROOT" | cut -d "-" -f 3` #"./NDK/android-ndk-r14" => "r14"
echo "NDK_VER = $NDK_VER"

#remove existing symbol link if link path is wrong
if [ -L $NDK_ROOT ]; then
  if [ "$(readlink -- "$NDK_ROOT")" != "/mtkoss/android-ndk/$NDK_VER/x86_64" ]; then
    rm -f $NDK_ROOT
  fi
fi

if [ ! -d $NDK_ROOT ]; then
  if [ -d /mtkoss/android-ndk/$NDK_VER/x86_64 ]; then
    ln -s "/mtkoss/android-ndk/$NDK_VER/x86_64" $NDK_ROOT
  else
    echo "Error: not found NDK in path: $NDK_ROOT"
    exit 1
  fi
fi
echo ""
