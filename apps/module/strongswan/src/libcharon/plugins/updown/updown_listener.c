	...
	bool up)
{
	...
	{	/* Config name: apn_xxx */
		...
		ike_sa->update_wdrv_keepalive_info(ike_sa, FALSE);
		notify_wod(N_DETACH, config->get_name(config) + 4, (void *)&this->prop);
	}
	else
	{
		if (this->prop.pcscf6[0][0] || this->prop.pcscf[0][0])
		{
			notify_wod(N_PCSCF,config->get_name(config) + 4, (void *)&this->prop);
		}
		if (this->prop.dns6[0][0] || this->prop.dns[0][0])
		{
		    notify_wod(N_DNS, config->get_name(config) + 4, (void *)&this->prop);
		}
		...
		notify_wod(N_ATTACH, config->get_name(config) + 4, (void *)&this->prop);
		ike_sa->update_wdrv_keepalive_info(ike_sa, TRUE);
	}
#endif
		...
		case 0:
			if(ike_sa->get_status_code(ike_sa) == PDN_STATUS_OK && ike_sa->get_pcscf_need_update(ike_sa))
			{
				if(pcscf)
				{
					if(config)
					{
						notify_wod(N_PCSCF_RESTORE, config->get_name(config) + 4, (void *)&this->prop);
