	...
	bool oos, u_int32_t oos_seq_num)
{
		...
		array_count(this->initiating.packets))
	{
			...
			!hydra->kernel_interface->get_data_path_status(hydra->kernel_interface))
		{
            ...
            else if (this->ike_sa->get_state(this->ike_sa) == IKE_CONNECTING)
            {
                DBG1(DBG_IKE, "stop retransmit in OOS when IKE is connecting state");
                goto RETRANSMIT_FAILED;
            }
		...
		if (!mobike || !mobike->is_probing(mobike))
		{
			...
			if (!mobike)
			{
				if (this->ike_sa->get_oos(this->ike_sa) && !this->ike_sa->has_condition(this->ike_sa, COND_STALE))
				{
					host_t *old, *new;
...
RETRANSMIT_FAILED:
	...
	if (this->ike_sa->get_oos(this->ike_sa))
	{
		...
		this->ike_sa->update_wdrv_keepalive_info(this->ike_sa, FALSE);
	}
	if (this->initiating.retransmitted == 0)
	{
		flush(this);
