	...
	linked_list_t *my_ts, linked_list_t *other_ts)
{
		...
		IS_MDMI_SUPPORTED())
	{
		...
		snprintf(tmp, sizeof(tmp), "%s,0x%08X,%H,%H,%u-%zu,0x%s,%u,0x%s",
				(src->get_family(src) == AF_INET)? "IPv4" : "IPv6",
				ntohl(spi), src, dst,
				enc_alg, encr.len * 8, enc_key_str,
				int_alg, int_key_str);
		notify_mdmi(N_ESP_DECRYPT_ADD, (void *)tmp);
	}
	   ...
	   private_child_sa_t *this)
{
	...
	if (this->my_spi)
	{
		...
		if (IS_MDMI_SUPPORTED())
		{
			...
			snprintf(tmp, sizeof(tmp), "0x%08X", ntohl(this->my_spi));
			notify_mdmi(N_ESP_DECRYPT_DEL, (void *)tmp);
		}
	}
	if (this->other_spi)
	{
		...
		if (IS_MDMI_SUPPORTED())
		{
			...
			snprintf(tmp, sizeof(tmp), "0x%08X", ntohl(this->other_spi));
			notify_mdmi(N_ESP_DECRYPT_DEL, (void *)tmp);
		}
	}
