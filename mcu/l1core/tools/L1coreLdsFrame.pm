...
{
    return " u0.03";
           # u0.03 , 2015/10/19 by Carl, Catch invalid input section
           # u0.02 , 2014/12/17 by Carl, Support multiple overlap section
           # u0.01 , 2014/03/31 by Carl, initial version
}
#****************************************************************************
# Constants
#****************************************************************************
...
sub GetPath
{
    ...
    my $strDefaultSystemFolder = "l1core/custom/system";
...
sub GenMEMORY
{
    ...
    my $MEMORYContent = &L1coreCommonUtil::GetFileContent($MEMORYPath);
    ...
    foreach my $i (@$MEMORY_SEGMENT_aref)
    {
        ...
        my $nBase = eval($i->[1]);
        my $nLen = eval($i->[2]);
        $g_MEMORYSetting{$i->[0]} = [&L1coreCommonUtil::Dec2Hex($nBase), 
                                     &L1coreCommonUtil::Dec2Hex($nLen)];
    }
    return $strMEMORY;
}
...
sub PreProcessRegionList
{
    ...
    my ($BasicRegionList_ref, $Index_ref) = &L1coreCommonUtil::ParseCSV($RegionListPath, undef);
...
sub GenSECTIONS
{
    ...
    foreach my $item (@$RegionList_ref)
    {
        ...
        elsif($strType =~ /^ACTION$/i or $strType =~ /^ENDACTION$/i)
        {
            ...
            my $strACTION = &L1coreCommonUtil::ConnetString(\@temp, ",", undef);
...
sub AvoidOrphanSECTIONS
{
    ...
    $strLayout = " "x4 ."\/\* If build fail with logs in link.log like this \n".
                 ...
                 " "x8 ."Please use correct input section name by following the rules in mcu/l1core/custom/system/Template/lds_config/InputSectionRule.txt\n".
                 " "x8 ."If you cannot find the symbol, please use \"\.\/m l1core gendummysym\" command.\n". 
                 " "x8 ."A dummy sym file will be generated in bin folder. Search the keyword INVALID_INPUT_SECTION to find the symbol\n".
                 " "x4 ."\*\/\n".
                 $strLayout if(0);	
...
sub GenBaseRegionName # Called by ProcessSection()
{
    ...
    my $strType = $Info_ref->[$Index_ref->{Type}];
    my $strName = ($strBaseRegion eq "" or $strType =~ /^OVERLAY$/i) ? $strOrgRegion : $strBaseRegion;
...
sub GenInputSections  # Called by ProcessSection()
{
    ...
    if(-e $HeadPath)
    {
        $InputSections = &L1coreCommonUtil::GetFileContent($HeadPath)."\n";
    }
    if(-e $SpecificChipPath)
    {
        $InputSections .= &L1coreCommonUtil::GetFileContent($SpecificChipPath)."\n";
    }
    elsif(-e $DefaultChipPath)
    {
        $InputSections .= &L1coreCommonUtil::GetFileContent($DefaultChipPath)."\n";
    }
    my $CommonPath = $InputSectionFolder.COMMON_InputSection.".txt";
    $InputSections .= &L1coreCommonUtil::GetFileContent($CommonPath);
...
sub GenInputSectionsByAttribute  # used by GenRORWInputSections and GenZIInputSections 
{
    ...
    if(scalar(@FileList)>0)
    {
        ...
        if(-e $strHeadPath)
        {
            $InputSections = &L1coreCommonUtil::GetFileContent($strHeadPath)."\n";
        }
        if(-e $strChipDependentPath)
        {
            $InputSections .= &L1coreCommonUtil::GetFileContent($strChipDependentPath)."\n";
        }
        elsif(-e $strDefaultChipPath)
        {
            $InputSections .= &L1coreCommonUtil::GetFileContent($strDefaultChipPath)."\n";
        }
        $InputSections .= &L1coreCommonUtil::GetFileContent($strCommonPath);
    }
    return $InputSections;
}
#****************************************************************************
...
sub Gen_TEMPLATE_LOADADDR  # Called by GenSECTIONS and Gen_TEMPLATE_OVERLAY()
{
    ...
    my $strOrgRegion = $Info_ref->[$Index_ref->{Name}];
    my $strType = $Info_ref->[$Index_ref->{Type}];
    return "" if($strBaseRegion ne "" and $strBaseRegion ne $strOrgRegion and $strType !~ /^OVERLAY$/i);
...
sub ldsFrame_die
{
    ...
    &L1coreCommonUtil::error_handler($error_msg, $file, $line_no, 'ldsFrame');    
}
#****************************************************************************
