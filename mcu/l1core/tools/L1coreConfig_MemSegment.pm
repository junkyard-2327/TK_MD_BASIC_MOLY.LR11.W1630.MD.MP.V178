...
sub Parse
{
    ...
    {
        &L1coreFileInfo::ParseDefinition($strFilePath, \%Results, 0);
        &_PostProcessFeatureConfig(\%Results, $INSIDE_MTK, $bIsDummy);
    }
    elsif($Source eq CUSTOM_IMG_CONFIG)
    {
        &L1coreFileInfo::ParseDefinition($strFilePath, \%Results, 1);
...
sub _PostProcessRamSize
{
    ...
    else
    {
        ...
        if(&L1coreFileInfo::is_NonSPNOR() and &exist('PROJECT_EXPECTED_RAM_LIMIT', FEATURE_CONFIG))
        {
            $Hash_ref->{nPROCESSED_RAMSIZE} = hex(&get('PROJECT_EXPECTED_RAM_LIMIT', FEATURE_CONFIG));
        }
        elsif(!&L1coreFileInfo::is_NonSPNOR() and &exist('PROJECT_EXPECTED_RAM_LIMIT_NFB', FEATURE_CONFIG))
        {
...
{
    my ($isDummy) = @_;
    return if($isDummy or !&L1coreFileInfo::is_NonSPNOR());
...
sub VerifyRAMInfo
{
    ...
    my $strRamLimit = (&L1coreFileInfo::is_NonSPNOR()) ? &get("PROJECT_EXPECTED_RAM_LIMIT", FEATURE_CONFIG)
...
sub Get_SDS_total_size
{
    ...
    if(!&L1coreFileInfo::is_NonSPNOR($feature_option_ref))
    {
...
sub GetMappingPath
{
    ...
    my $strBoard = &L1coreFileInfo::get("BOARD_VER");
    my $strFlavor = &L1coreFileInfo::get("FLAVOR");
    my $strPath = "pcore/custom/system/$strBoard/$strFlavor/$strFileName\.conf";
    return $strPath;
}
sub is
{
...
sub config_die
{
    ...
    &L1coreSysUtil::error_handler($error_msg, $file, $line_no, '[Memory Config]');
}
...
{
    my ($file, $line_no) = @_;
    &config_die("This function is unsupported on non NOR platform.", $file, $line_no) if(!&L1coreFileInfo::is_NonSPNOR());
}
