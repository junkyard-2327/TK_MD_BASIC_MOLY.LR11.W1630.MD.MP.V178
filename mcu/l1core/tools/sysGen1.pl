...
my $TemplateDirPath  = 'l1core/custom/system/Template';
...
#****************************************************************************
opendir( my $DIR, $TemplateDirPath) || &L1coreSysUtil::sysgen_die("Can't open $TemplateDirPath!", __FILE__, __LINE__);
...
foreach my $file (@Templates)
{
    ...
    {
        &L1coreSysUtil::del_noCheckinHistory_file($FilePathInProject);
    }
...
sub ReportRepeatedFiles
{
    ...
    foreach my $item (@temp_array)
    {
        ...
        foreach my $filename (@$FileNames_ref)
        {
            if($filename eq $item)
            {
                &L1coreSysUtil::sysgen_die("$item and $item.template can't exist at the same time!", __FILE__, __LINE__);
                last;
            }
        }
    }
