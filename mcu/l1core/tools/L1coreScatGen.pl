...
#****************************************************************************
#&L1coreSysUtil::del_noCheckinHistory_file($BL_SCATTERFILE);
#&L1coreSysUtil::del_noCheckinHistory_file($EXT_BL_SCATTERFILE);
&L1coreSysUtil::del_noCheckinHistory_file($SCATTERFILE);
&L1coreSysUtil::del_noCheckinHistory_file($DL_SCATTERFILE);
...
my %CmplOption;
my %RelOption;
L1coreFileInfo::PreProcess($themf, \%MAKEFILE_OPTIONS, $INFOMAKELOG, \%CmplOption, \%RelOption,1);
FileInfo::PreProcess($themf, \%MAKEFILE_OPTIONS, $INFOMAKELOG, \%CmplOption, \%RelOption,1);     
$g_strCOMPILER = &L1coreFileInfo::GetCompiler();
$g_bb = &L1coreSysUtil::SwitchToClonedChip(&L1coreFileInfo::GetChip()); #use infomake.log

#****************************************************************************
# 4 >>> Check user settings validity - DUMMY SCATTER
#****************************************************************************
$g_bUSE_DUMMY_SCATTER = &L1coreSysUtil::isDummyScatter($INSIDE_MTK, $DUMMY_SCATTER_FORCE);
...
#****************************************************************************
if (0 && !-e $BL_SCATTERFILE and $NEED_BUILD_BOOTLOADER eq "TRUE")
{
...
# input:
# return:      x
# require:    pcore/tools/ldsGenLib.pl -> ldsGen_main();
#****************************************************************************
sub GenMAUI_LDS
{
    my ($filepath, $bIs_Flashtool_Layout) = @_;
    require "L1coreLdsGenLib.pl";
    ...
    &Output($filepath, "defines the memory map for the validation board", 
         &tclGen_verno(), &get_board_parameters(), 1, $strLayout);
}
...
# return:      x
# require:    pcore/tools/BLscatGen.pl -> scatGenBL_main()
#****************************************************************************
sub wait_for_resue
#sub GenBLScatter
{
    require "pcore/tools/scatGenBL.pl";
    my ($strBL, $strEXTBL) = &scatGenBL_main(sub { my ($key) = @_;
                                      return &L1coreFileInfo::get($key);
                                    },
                               $g_bb,
                               0, # ram size
                               $BB_PATH,
...
sub Output
{
    ...
    open (FILE, ">$strFilePath") or &L1coreSysUtil::sysgen_die("$strFilePath: open file error!", __FILE__, __LINE__);
    print FILE &L1coreSysUtil::copyright_file_header();
    print FILE &L1coreSysUtil::description_file_header($strFilePath,
                                                 $description."\n",
...
# input:
# return:      x
# require:    pcore/tools/ldsGenBL.pl -> BLldsGen_main();
#****************************************************************************
...
#sub GenBL_LDS
{
    ...
    require "pcore/tools/ldsGenBL.pl";
...
sub CopyandMarkFile
{
    ...
    my $content;
    open FILE, "<$SrcPath" or &L1coreSysUtil::sysgen_die("$SrcPath: open file error!", __FILE__, __LINE__);
    {
        local $/;
        $content = <FILE>;
    }
    ...
    open FILE, ">$DestPath" or &L1coreSysUtil::sysgen_die("$DestPath: open file error!", __FILE__, __LINE__);
    print FILE $content;
    close FILE;
}
#****************************************************************************
...
sub DelFileWithKeyword
{
    ...
    if(-e $FilePath)
    {
        ...
        open FILE, "<$FilePath" or &L1coreSysUtil::sysgen_die("$FilePath: open file error!", __FILE__, __LINE__);
        {
            local $/;
            my $content = <FILE>;
...
{
    my ($loadsize, $ramsize) = ( 0, # rom size
                                 0, # ram size
                               );
...
#    if (!L1coreFileInfo::is_NonSPNOR()) { return $template2; }
#    else                          
    { return $template1; }
}
...
{
    my $strSWCFG;
    if(&L1coreFileInfo::isnot('nand_flash_booting', 'NONE'))
    {
        $strSWCFG .= "NFB ";
    }
    if(&L1coreFileInfo::isnot('emmc_booting', 'NONE'))
    {
        $strSWCFG .= "EMB ";
    }
#    if(&L1coreFileInfo::is_SmartPhone())
#    {
#        $strSWCFG .= "[SmartPhone]". &L1coreFileInfo::get("SMART_PHONE_CORE"). " ";    
#    }
    if(&L1coreFileInfo::isnot('usb_download_in_bl', 'NONE'))
    {
        $strSWCFG .= "USBDOWNLOAD ";
    }
    if(&L1coreFileInfo::is('multiple_binary_files', 'TRUE'))
    {
        $strSWCFG .= "MULTIBIN ";
    }
    if(&L1coreFileInfo::is('secure_support', 'TRUE'))
    {
        $strSWCFG .= "SECURE ";
    }
    if(&L1coreFileInfo::is('enhanced_single_bank_nor_flash_support', 'TRUE'))
    {
        $strSWCFG .= "ESBNOR ";
    }
    return $strSWCFG;
...
sub description_file_header_for_scatter
{
    ...
    my @lines = split(/\n/, &L1coreSysUtil::description_file_header($filename, $description, $author, $board_description, 1));
    my $template;
    map{ $template .= ";".$_."\n" }@lines;
    return $template;
}
...
sub copyright_file_header_for_scatter
{
    my ($bb) = @_;
    my $pre_compile_str;
    if(&L1coreFileInfo::is("dsp_solution", "DUALMACDSP") or $bb eq "MT6251")
    {
        $pre_compile_str = "#! armcc -E";
    }
    my @lines = split(/\n/, &L1coreSysUtil::copyright_file_header());
    my $template;
    map{ $template .= ";".$_."\n" }@lines;
...
#        print "Processed RamSize = ". GetExtRamSizetemp().
#                                  "(".&CommonUtil::Dec2Hex(GetExtRamSizetemp()).")\n";
#        print "Processed ROMSize = ". GetROMSizetemp().
#                                  "(".&CommonUtil::Dec2Hex(GetROMSizetemp()).")\n";
#        print "\n";
    }
}
sub CheckDummyScatter
{
    ...
    if(&L1coreSysUtil::HasCheckinHistory($strScatterPath) && $bIsDummy)
    {
        &L1coreSysUtil::sysgen_die("Generating DUMMY SCATTER FILE can't work when a manual checked-in scatter file exists.\nRecommand: Please remove your own modifications first, \nrebuild and make sure the load is successfully built, \nthen add your own modifications back.\n", __FILE__, __LINE__);
    }
}
