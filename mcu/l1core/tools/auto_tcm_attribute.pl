...
{  
   ...
   {  
      ...
      {      
         $ITCM_LIST{$1} = 0;         
         $ITCM_LIST_SECTION{$1} = (scalar(@section_adt)==0)? $section : "$section_adt[0],$section_adt[1],$section_adt[2]";
         #printf "[ADT_DBG] $ITCM_LIST_SECTION{$1}\t$1\n";
      }   
   }
   ...
   while( <FILE_IMM> )
   {
      ...
      if($hited_target && $line_in =~ /([{;])/ )
      {
         if( $1 eq '{' )  # Function body is detected.
         {
            ...
            if( $hited_is_adt == 0 )
            {
               $ADDED_ATTRIBUTE{ $hited_lineno } = $hited_section; #$ITCM_LIST_SECTION{ $hited_target };
