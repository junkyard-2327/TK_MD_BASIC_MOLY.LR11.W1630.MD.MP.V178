...
{
    return " u0.01_UMOLY";
           # u0.02 , 20150806 by Carl, Avoid taking linker symbol with postfix "_LENGTH" as a section
           # u0.01 , 20140401 by Carl, initial version
}
...
sub parse_linker_symbol #used by ParseSECTIONS
{
        ...
        $strLine =~ /Image\$\$(\S+)\$\$(\S+)\s*\=\s*\s*(\S+|\.)\s*\;/)
    {
        ...
        return if ($strRegionName =~ /LENGTH/ ); # should not add any LENGTH in section name
