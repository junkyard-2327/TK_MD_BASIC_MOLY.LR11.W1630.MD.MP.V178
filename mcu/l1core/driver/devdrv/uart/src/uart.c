 ...
 * $Log$
 *
 * 12 08 2016 flamingo.wang
 * [MOLY00217631] [Rosa][Scan Result Tracking]add rosa chip define to UART driver code
 * add 6750S define
 *
 * 08 03 2015 yuke.ren
...
#define UART_DMA_REMAP 0

#if defined(MT6595_S00)
#define P_UART_ECPT_BUFFER (kal_uint8*)virt_to_phys((void*)uart_ecpt_bps_buf)
#else
#define P_UART_ECPT_BUFFER uart_ecpt_bps_buf
#endif
...
#if defined(MT6595_S00) && UART_DMA_REMAP
			qbm_tx_des_is_cached = INT_QueryIsCachedRAM(p_ior_head,1);
...
kal_bool uart_except_init_qbm()
{
	...
	conf.p_mem_pool_str = P_UART_ECPT_BUFFER;
	conf.p_mem_pool_end = P_UART_ECPT_BUFFER + UART_ECPT_QBM_BPS_BUF_SZ;
...
{
    if ((kal_uint8 *)p_gpd >= P_UART_ECPT_BUFFER && 
        (kal_uint8 *)p_gpd < P_UART_ECPT_BUFFER + UART_ECPT_QBM_BPS_BUF_SZ)
        return KAL_TRUE;
