...
#include "us_timer.h"
#if (defined(MT6290) || defined(MT6290)) && defined(__ARM7EJ_S__)
...
extern void ReEnableIRQFIQ(void);
...
extern kal_int32 TCD_Highest_Priority_HISR;
extern kal_uint32 TimerHISR_Preempted_Time;
#endif
/*************************************************************************
...
#if ((defined(MT6290) || defined(TK6291) || defined(MT6755) || defined(MT6797) || defined(MT6757) || defined(MT6750S)) && defined(__ARM7EJ_S__)) 
...
//avoid warming cause error for l1core build.
//notice re-open it if need, to register hisr in l1core_hisr_config_internal.h
#if 1
   /* Set CTIRQ1 handler */
...
//   kal_hisr processing_ehisrid;
   ...
   kal_uint32 TimerHISR_Exclude_End_Time = 0;
...
//      ASSERT(NULL != processing_hisrid);
   ...
   if(CheckIRQLock())
   {
       kal_fatal_error_handler(KAL_ERROR_INTERRUPT_DISABLED_AFTER_LISR_FAILED, (kal_uint32)processing_lisr);
   }
   LockIRQ();
   processing_irqx = processing_irqx_backup;
   processing_lisr = processing_lisr_backup; 
#if defined(DEBUG_KAL) && defined(DEBUG_TIMER) && defined(__CR4__)
   if(TimerHISR_State == 2 && 
      ESAL_GE_ISR_Executing == 1 && 
      TCD_Highest_Priority_HISR == 2)
   {
       GET_CURRENT_TIME(TimerHISR_Exclude_End_Time);
       TimerHISR_Preempted_Time += (TimerHISR_Exclude_End_Time - TimerHISR_Exclude_Start_Time);
       TimerHISR_State = 1;
   }
#endif
#endif /* __UNIFIED_ISR_LEVEL__ */
...
/* For MT6268 2G/3G or 2G Single project: timer interrupt is moved to l1csm.c for Dual/Single OS Tick mechanism  */
   ...
   /*KTEST: start timer tick in target full load if not event based timer!*/
#elif defined(__KTEST__) && !defined(__MAUI_BASIC__) && !defined(__CENTRALIZED_SLEEP_MANAGER__)
