 ...
 * $Log$
 *
 * 11 24 2016 yen-chun.liu
 * [MOLY00214694] [LR11][MP5][MDCIRQ] ROSA call for check-in
 * ROSA MDCIRQ call for check-in.
 *
 * 02 16 2016 i-chun.liu
 * [MOLY00164979] MT6757 Call for check-in
 * MT6757 call for check-in: L1CORE MDCIRQ.
 *
 * 02 16 2016 i-chun.liu
 * [MOLY00164979] MT6757 Call for check-in
 * MT6757 call-for-check-in: L1CORE MDCIRQ..
 ...
 * [MOLY00142967] Update L1core MDCIRQ ungate
 * Update: Use pdn driver to ungate L1 MDCIRQ.
 *
 * 08 04 2015 i-chun.liu
 * [MOLY00135138] MT6797 bring up call for check in
 * MT6797 L1core MDCIRQ call for bring up.
 *
 * 06 09 2015 i-chun.liu
 * [MOLY00119728] JADE bring up call for check in  (MDCIRQ and Bus monitor )
 * JADE L1CORE MDCIRQ.
 *
 * 03 25 2015 i-chun.liu
 * [MOLY00085849] Update UMOLY DEV/TRUNK CIRQ driver (mainly CIRQ table and priority)
 * Modify L1core Sleep mask value.
 *
 * 03 04 2015 i-chun.liu
 * [MOLY00085849] Update UMOLY DEV/TRUNK CIRQ table and priority
 * Ungate L1core MDCIRQ modification .
 *
 ...
 *
 * 06 03 2014 yh.peng
 * [MOLY00067412] [TK6291][SystemService] Bring Up check in.Check in for TK6291_DEV L1@TCM bring-up success.
 *
 * 05 13 2014 linson.du
 * [MOLY00061877] [TK6291]driver initial porting for TK6291
 * Fix a CIRQ driver bug. vec offset = size>>2 -1
 *
 * 05 07 2014 linson.du
 * [MOLY00061877] [TK6291]driver initial porting for TK6291
 * Update L1core CIRQ ID.
 *
...
const kal_uint32 cirq_dump_regions[] = {
   ...
   MDCIRQ_BASE,    0x010C,        4,     /* CIRQ */
   (MDCIRQ_BASE+0x290),    0x0008,        4,     /* CIRQ vector address base */
   (MDCIRQ_BASE+0x490),    0x0008,        4,     /* CIRQ vector address default */
#else
   0, 0, 0
#endif
};
...
#if (defined(TK6291) || defined(MT6755) || defined(MT6797) || defined(MT6757) || defined(MT6750S))
	DRV_WriteReg32(MDCIRQ_VEC_BASE, ((kal_uint32)&ISRRoutines));
	ISR_Routine_size = (ISR_Routine_size>>2) - 1;
	DRV_WriteReg32(MDCIRQ_VEC_OFFSET, ((kal_uint32)ISR_Routine_size));

#if !defined(__UNIFIED_ISR_LEVEL__)
	for(i = 0; i < MAX_ISR_NUM/4; i++)
	{
		/* Set Interrupt Priority */
		priroty_combine = irq_priority_list[4*i] | irq_priority_list[4*i + 1] << 8 | irq_priority_list[4*i+2]<<16 | irq_priority_list[4*i+3]<<24;
		DRV_WriteReg32(MDCIRQ_PRLV(i<<2), priroty_combine);
	}
#endif /* __UNIFIED_ISR_LEVEL__ */
...
void drv_mdcirq_spl_init(void)
{
	...
	for(i = 0; i < MAX_ISR_NUM/4; i++)
	{
		intID = irq_priority_list[4*i] | irq_priority_list[4*i + 1] << 8 | irq_priority_list[4*i+2]<<16 | irq_priority_list[4*i+3]<<24;
		DRV_WriteReg32(MDCIRQ_PRLV(i<<2), intID);
	}
...
#if defined(IRQ_OSTIMER_WAKEUP_CODE)
   SetMaskValue(&SleepModeMask,IRQ_OSTIMER_WAKEUP_CODE); 
#endif

#if defined(IRQ_WTIMER_WAKEUP_CODE)
   SetMaskValue(&SleepModeMask,IRQ_WTIMER_WAKEUP_CODE);
#endif

#if defined(IRQ_TDD_WAKEUP_CODE)
   SetMaskValue(&SleepModeMask,IRQ_TDD_WAKEUP_CODE);
#endif

#if defined(IRQ_TDMA_WAKEUP_CODE)
   SetMaskValue(&SleepModeMask,IRQ_TDMA_WAKEUP_CODE);
#endif

#if defined(IRQ_LTE_WAKEUP_CODE)
   SetMaskValue(&SleepModeMask,IRQ_LTE_WAKEUP_CODE);
#endif


//invalid IRQ
#if 0//defined(IRQ_RTR_SLT_CODE)&&(MDCIRQ_128_64_VERSION==1)
/* under construction !*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
//invalid IRQ
#if 0//defined(IRQ_WTIMER_CODE)&&(MDCIRQ_128_64_VERSION==1)
/* under construction !*/
/* under construction !*/
#endif

#if 0//def IRQ_OSTIMER_ARM_CODE
/* under construction !*/
#endif

#if 0//defined(ATEST_DRV_ENABLE)&&defined(IRQ_RTC_CODE)
/* under construction !*/
#endif

#if 0//defined(IRQ_TDMA_CODE)&&(MDCIRQ_128_64_VERSION==1)
/* under construction !*/
#endif
...
void initINTR(void)
{
   ...
   //Ungate L1core MDCIRQ
#if defined(TK6291) //hardcode for TK6291 since driver is not ready 
   DRV_WriteReg32(0xF60F0008,0x2000);
   ...
   cirq_dump.bbreg_dump_callback = NULL;
