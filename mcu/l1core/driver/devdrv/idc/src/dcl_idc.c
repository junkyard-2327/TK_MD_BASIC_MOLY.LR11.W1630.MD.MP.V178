...
DCL_STATUS DclIDC_Control(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	...
	switch(cmd)
	{
		...
		case IDC_CMD_SEND_EVENT:
		{
			...
			drv_idc_send_event(pr_ctrl_schedule_event->num_event, (IDC_EVENT_T *) &(pr_ctrl_schedule_event->schedule_event), pr_ctrl_schedule_event->sleep_mode);
			return_value = STATUS_OK;
			break;
		}
		...
		case IDC_CMD_SCHEDULE_START:
		{
			...
			drv_idc_set_global_offset(pr_ctrl_schedule_start->schedule_num, pr_ctrl_schedule_start->global_offset);
			return_value = STATUS_OK;
			break;
		}
