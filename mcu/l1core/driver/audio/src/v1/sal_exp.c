...
DEFINE_NC_SHARED_VAR(kal_uint32, su4EVS_Switch);
DEFINE_NC_SHARED_VAR(kal_uint32, su4EVS_Init);
#else
...
void SAL_2G_Call_Open(uint32 enc_mod, uint32 dec_mod, uint32 sub_channel, uint32 tch_state)
{
	...
	SALI_2G_Codec_Protect(&enc_mod, &dec_mod);
...
void SAL_2G_Handover(uint32 enc_mod, uint32 dec_mod, uint32 sub_channel, uint32 tch_state)
{
        ...
        L1Audio_Msg_AM_Handover( enc_mod, sub_channel );
...
void SAL_3G_Call_Open(uint32 enc_mod, uint32 dec_mod, uint32 dtx, uint32 delR, uint32 delW, uint32 delM)
{
	...
	SALI_AppSpec_Control(SAL_APP_TYPE_3GCall, 1);
   	...
   	SALI_VBI_Reset();
#ifdef SAL_EVS_STAND_ALONE
	SHARED_VAR(su4EVS_Switch) = 1;
	SHARED_VAR(su4EVS_Init) = 1;
#endif 	  
}
...
void SAL_4G_ExtCod_Call_Open(uint32 band, Sal_ExtCod_t Cod,uint32 delR, uint32 delW, uint32 delM)
{
	...
	SALI_Config_SRC_Extend(band);	
	SALI_Set_Sch_Delay(delR, delW, delM, 0, SAL_UNKNOWN);
	SALI_AppSpec_Control(SAL_APP_TYPE_4GCall, 1);
	SALI_VBI_Reset();
	SHARED_VAR(su4EVS_Enc_Init) = 1;
	SHARED_VAR(su4EVS_Dec_Init) = 1;
}

void SAL_4G_ExtCod_Call_Close(uint32 dsp_end)
{
	L1Audio_Msg_SAL_PROC(SAL_PROC_NAME(39), dsp_end, SAL_UNKNOWN, SAL_UNKNOWN, SAL_UNKNOWN);
	if(dsp_end)
		SALI_VBI_End();
	SALI_AppSpec_Control(SAL_APP_TYPE_4GCall, 0);
}
...
void SAL_OpenDSP_Bypass_MD(bool UL, bool DL)
{
	SALI_OpenDSP_Bypass_MD(UL, DL);
}
...
uint16 SAL_OpenDSP_Get_Polling_Time(bool UL, bool DL)
{
    return SALI_OpenDSP_Get_Polling_Time(UL,DL);
}