...
/*If __ENFORCE_HW_US_TIMER__ is defined, system enforce using UST for Qbit checking. 
  Now Pcore __ENFORCE_HW_US_TIMER__ is defined since Pcore DVFS is enabled          */
#if defined(__PCORE__)
#define __ENFORCE_HW_US_TIMER__
#endif 
...
#if (defined(__CR4__) && !defined(__ENFORCE_HW_US_TIMER__))
...
#define IRQ_DISABLE_MAX_DURATION    ((unsigned int)((((SST_HR_DUR*12*CPU_CLOCK_MHZ*30)/13))/0.95))
#else
...
#if defined(TK6291) && defined(__FPGA__)
#define IRQ_DISABLE_MAX_DURATION    (56*30)    /* (60/13)*12*30=1680 us */
#else
...
#if defined(TK6291) && defined(__FPGA__)
#define IRQ_DISABLE_MAX_DURATION    SST_HR_DUR*30    /* 1800 qbit */
#else
#define IRQ_DISABLE_MAX_DURATION    SST_HR_DUR    /* 60 qbit */
#endif
