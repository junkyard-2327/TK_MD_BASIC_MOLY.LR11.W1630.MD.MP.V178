...
# -----------------------------
-include ${COMMON_ROOT}/service/dhl/database/FeatureSetting.ini
...
ifeq ($(strip $(CUSTOM_RELEASE)),TRUE)
  ifeq ($(strip $(DHL_SUPPORT)),TRUE)
    BPLGUINFOCUSTOM = $(notdir $(wildcard $(CUS_MTK_REL)/dhl/database/pcore_db/$(strip $(BPLGUINFOCUSTOMPREFIX))*))
  else
...
ifeq ($(strip $(MODIS_CONFIG)),TRUE)
  BPLGUINFOCUSTOM = MDDB.P_pc.EDB
endif

ifeq ($(strip $(TEST_LOAD_TYPE)),L1S)
  ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
    CODEGEN_RESULT = $(strip $(MODISBUILD_TST_DB))/MDDB.P.EDB
  endif
else
...
ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
  ifneq ($(strip $(TEST_LOAD_TYPE)),L1S)
    CODEGEN_DATABASE_OUT = $(strip $(MODISBUILD_TST_DB))/pcore_db/$(strip $(BPLGUINFOCUSTOM))
    ifeq ($(strip $(DHL_SUPPORT)),TRUE)
      CODEGEN_DATABASE_OUT_SEC = $(subst .EDB,_sec.EDB,$(strip $(MODISBUILD_TST_DB_SEC))/$(strip $(BPLGUINFOCUSTOM)))
    endif
  else
    CODEGEN_DATABASE_OUT = $(strip $(MODISBUILD_TST_DB))/pcore_db/MDDB.P.EDB
    ifeq ($(strip $(DHL_SUPPORT)),TRUE)
      CODEGEN_DATABASE_OUT_SEC = $(strip $(MODISBUILD_TST_DB_SEC))/MDDB.P_sec.EDB
    endif
  endif
else
  CODEGEN_DATABASE_OUT = $(strip $(MODISBUILD_TST_DB))/pcore_db/$(strip $(BPLGUINFOCUSTOMSRCP))
  ifeq ($(strip $(DHL_SUPPORT)),TRUE)
    ...
    ifneq ($(strip $(TEST_LOAD_TYPE)),L1S)
      ifeq ($(strip $(MODIS_CONFIG)),TRUE)
        ifeq ($(strip $(call Upper,$(UE_SIMULATOR))),TRUE)
          CODEGEN_DATABASE_IN_SEC = $(wildcard $(CUS_MTK_REL)/dhl/lte_sec/database_uesim/$(subst .EDB,_sec.EDB,$(strip $(BPLGUINFOCUSTOM))))
        else
          CODEGEN_DATABASE_IN_SEC = $(wildcard $(CUS_MTK_REL)/dhl/lte_sec/database_modis/$(subst .EDB,_sec.EDB,$(strip $(BPLGUINFOCUSTOM))))
        endif
      else
...
ifneq ($(strip $(TEST_LOAD_TYPE)),L1S)
  ifneq ($(strip $(AUTO_CHECK_CGEN)),TRUE)
    ifneq ($(strip $(MODIS_CONFIG)),TRUE)
      ifeq ($(strip $(DHL_SUPPORT)),TRUE)
        CODEGEN_DATABASE_IN = $(CUS_MTK_REL)/dhl/database/pcore_db/$(strip $(BPLGUINFOCUSTOM))
      else
        CODEGEN_DATABASE_IN = $(CUS_MTK_REL)/tst/database/$(strip $(BPLGUINFOCUSTOM))
      endif
    else
      ifeq ($(strip $(DHL_SUPPORT)),TRUE)
        CODEGEN_DATABASE_IN = $(CUS_MTK_REL)/dhl/database_modis/pcore_db/$(strip $(BPLGUINFOCUSTOM))
      else
...
ifneq ($$(strip $$(MODIS_CONFIG)),TRUE)
  ...
  ifeq ($$(strip $$(MODIS_COMPILER)),VC2008)
	...
	@perl ${CORE_ROOT}/tools/pack_dep_gen.pl $$@ $$@ $$(patsubst %/,%,$$(dir $$@)) "\b$$(basename $$(notdir $$@))\.log"
...
code_generate: sysgen cleancodegen eas_gen asngen umts_gen codegen merge_db asnregen cleanbin mcddll_update send
    else
code_generate: sysgen cleancodegen eas_gen asngen umts_gen codegen asnregen cleanbin mcddll_update send
  ...
  # [TATAKA Migrate] single-mode
    ifneq ($(SINGLE_CORE),TRUE)
code_generate: sysgen cleancodegen eas_gen asngen codegen merge_db cleanbin mcddll_update_ltesm send
    else
...
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
	...
	(perl uas_generate.pl --CC="$(CC)" --COMPILER=$(COMPILER) --feature=" $(UMTS_RELEASE_SUPPORT) UMTS_FDD_MODE_SUPPORT $(ULCS_ASN_SUPPORT_VERSION) $(REMOVE_DEBUG_INFO) " --deffile="../../../$(INFOLOG)" --tst_src_path="$(MODIS_TST_DB)" --tst_dst_path="$(MODISBUILD_TST_DB)" --targetdir="$(PROJDIR)" --log_dir="$(strip $(COMPLOGDIR))")
    # TDD umts_gen
	@$(CHDIR) $(strip $(UAS_TDD_FOLDER)) $(AND) \
	(perl uas_generate.pl --CC="$(CC)" --COMPILER=$(COMPILER) --feature=" $(UMTS_RELEASE_SUPPORT) UMTS_TDD128_MODE_SUPPORT $(ULCS_ASN_SUPPORT_VERSION) $(REMOVE_DEBUG_INFO) " --deffile="../../../$(INFOLOG)"  --tst_src_path="$(MODIS_TST_DB)" --tst_dst_path="$(MODISBUILD_TST_DB)" --targetdir="$(PROJDIR)" --log_dir="$(strip $(COMPLOGDIR))")
else
    # FDD umts_gen
	@$(CHDIR) $(strip $(UAS_FDD_FOLDER)) $(AND) \
	(perl uas_generate.pl --CC="$(CC_MODIS)" --COMPILER=$(MODIS_COMPILER) --feature=" $(UMTS_RELEASE_SUPPORT) UMTS_FDD_MODE_SUPPORT $(ULCS_ASN_SUPPORT_VERSION) MoDIS " --deffile="../../../$(INFOLOG_MODIS)" --tst_src_path="$(MODIS_TST_DB)" --tst_dst_path="$(MODISBUILD_TST_DB)" --targetdir="$(PROJDIR)" --log_dir="$(strip $(MODISLOGDIR))")
    # TDD umts_gen
	@$(CHDIR) $(strip $(UAS_TDD_FOLDER)) $(AND) \
	(perl uas_generate.pl --CC="$(CC_MODIS)" --COMPILER=$(MODIS_COMPILER) --feature=" $(UMTS_RELEASE_SUPPORT) UMTS_TDD128_MODE_SUPPORT $(ULCS_ASN_SUPPORT_VERSION) MoDIS " --deffile="../../../$(INFOLOG_MODIS)"  --tst_src_path="$(MODIS_TST_DB)" --tst_dst_path="$(MODISBUILD_TST_DB)" --targetdir="$(PROJDIR)" --log_dir="$(strip $(MODISLOGDIR))")
endif
# mbis time probe
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_E,$@,T," >> $(MBIS_BUILD_TIME_TMP)
...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(CHDIR) ${CORE_ROOT}/modem/l4/csm/ss/tools $(AND) \
...
ifeq ($(strip $(RUN_LTE_MCDDLL_ONLY_COPY)),FALSE)
	...
	@$(MAKE) -fmake/${CORE_ROOT}/build.mak -r -R --no-print-directory  GAS_FOLDER=$(strip $(CORE_ROOT))/modem/gas UAS_FOLDER=$(strip $(CORE_ROOT))/modem/uas XGEN_SWITCH_FDD_TDD=FDD $(strip $(TMPDIR))/~lte_mcddll_inc.tmp
...
# TARGET: temp add
# *************************************************************************
$(strip $(TMPDIR))/~mcddll_inc.tmp $(strip $(TMPDIR))/~lte_mcddll_inc.tmp: FORCE
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
  ifeq ($(strip $(COMPILER)),GCC)
...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(STRCMPEX) def def e $(strip $(TMPDIR))/~def.tmp $(foreach def,$(sort $(COM_DEFS)),$(def))
	@$(LNITEM) $(strip $(TMPDIR))/~def.tmp
	@$(MAKE) -fmake/${CORE_ROOT}/build.mak -r -R --no-print-directory  GAS_FOLDER=$(strip $(CORE_ROOT))/modem/gas UAS_FOLDER=$(strip $(CORE_ROOT))/modem/uas XGEN_SWITCH_FDD_TDD=FDD $(strip $(TMPDIR))/~mcddll_inc.tmp
...
ifeq ($(strip $(RUN_MCDDLL_UPDATE)),TRUE)
  ifneq ($(strip $(MODIS_CONFIG)),TRUE)
	@$(CHDIR) ${CORE_ROOT} $(AND) \
	(perl ./modem/common/mcddll/updatemcddll_mingw.pl --PROJECT="$(strip $(PROJECT_MAKEFILE))" --L4=0 --MODE="TARGET" --VERNO=$(strip $(VERNO)) --HW_VERSION=$(strip $(PLATFORM))_$(strip $(CHIP_VER)) --PROJDIR=$(PROJDIR) --BUILD_L4_DIR=$(BUILD_L4DIR) --MINGW_DIR=$(MinGWDir) --COMPLOGDIR=$(COMPLOGDIR) 2> ../$(MCDDLL_UPDATE_LOG) \
	$(WITH) \
	$(IF_ERR_TRUE) $(THEN) (echo Error: $@ Failed! Please check $(strip $(MCDDLL_UPDATE_LOG)) $(WITH) exit 1) $(CMD_END))
  else
	@$(CHDIR) ${CORE_ROOT} $(AND) \
	(perl ./modem/common/mcddll/updatemcddll_mingw.pl --PROJECT="$(strip $(PROJECT_MAKEFILE))" --L4=0 --MODE=MoDIS --VERNO=$(strip $(VERNO)) --HW_VERSION=$(MODIS_UESIM) --PROJDIR=$(PROJDIR) --BUILD_L4_DIR=$(BUILD_L4DIR) --MINGW_DIR=$(MinGWDir) --COMPLOGDIR=$(MODISLOGDIR) 2>../$(MCDDLL_UPDATE_LOG) \
	$(WITH) \
	$(IF_ERR_TRUE) $(THEN) (echo Error: $@ Failed! Please check $(strip $(MCDDLL_UPDATE_LOG)) $(WITH) exit 1) $(CMD_END))
  endif
	@$(COPY_FILE) $(strip $(PROJDIR))/modem/mcddll/mcddll.dll $(strip $(MODISBUILD_TST_DB))/mcddll.dll >$(DEV_NUL)
  ...
  ifneq ($(strip $(MODIS_CONFIG)),TRUE)
	@$(CHDIR) ${CORE_ROOT} $(AND) \
	(perl ./modem/common/mcddll/updatemcddll_mingw.pl --PROJECT="$(strip $(PROJECT_MAKEFILE))" --L4=1 --MODE="TARGET" --VERNO=$(strip $(VERNO)) --HW_VERSION=$(strip $(PLATFORM))_$(strip $(CHIP_VER)) --PROJDIR=$(PROJDIR) --BUILD_L4_DIR=$(BUILD_L4DIR) --REL_DIR=$(CUS_MTK_REL) --MINGW_DIR=$(MinGWDir) --COMPLOGDIR=$(COMPLOGDIR) 2>../$(MCDDLL_UPDATE_LOG) \
	$(WITH) \
	$(IF_ERR_TRUE) $(THEN) (echo Error: $@ Failed! Please check $(strip $(MCDDLL_UPDATE_LOG)) $(WITH) exit 1) $(CMD_END))
  else
	@$(CHDIR) ${CORE_ROOT} $(AND) \
	(perl ./modem/common/mcddll/updatemcddll_mingw.pl --PROJECT="$(strip $(PROJECT_MAKEFILE))" --L4=1 --MODE=MoDIS --VERNO=$(strip $(VERNO)) --HW_VERSION=$(MODIS_UESIM) --PROJDIR=$(PROJDIR) --BUILD_L4_DIR=$(BUILD_L4DIR) --REL_DIR=$(CUS_MTK_REL) --MINGW_DIR=$(MinGWDir) --COMPLOGDIR=$(MODISLOGDIR) 2>../$(MCDDLL_UPDATE_LOG) \
	$(WITH) \
	$(IF_ERR_TRUE) $(THEN) (echo Error: $@ Failed! Please check $(strip $(MCDDLL_UPDATE_LOG)) $(WITH) exit 1) $(CMD_END))
  endif
	@$(COPY_FILE) $(strip $(PROJDIR))/modem/mcddll/mcddll.dll $(strip $(MODISBUILD_TST_DB))/mcddll.dll >$(DEV_NUL)
...
ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
  ifeq ($(filter BASIC UDVT,$(TEST_LOAD_TYPE)),)
		...
		(perl ./scsi_generate.pl --CC=$(CC) --COMPILER=$(COMPILER) --feature=" $(UMTS_RELEASE_SUPPORT) " --PROJDIR=$(PROJDIR)) 
...
ifeq ($(strip $(MODIS_CONFIG)),TRUE)
	...
	@$(MAKE) -fmake/${CORE_ROOT}/build.mak -r -R $(CMD_ARGU) --no-print-directory AUTO_CHECK_DEPEND=$(AUTO_CHECK_DEPEND) AUTO_CHECK_CGEN=$(AUTO_CHECK_CGEN) MODIS_CONFIG=$(strip $(MODIS_CONFIG)) MODIS_UESIM=$(strip $(MODIS_UESIM)) genmoduleinfo_start
...
ifneq ($(filter __MOD_NVRAM__,$(strip $(COM_DEFS))),)
  ...
  ifeq ($(strip $(NVRAM_LID_CHECK)),TRUE)
		...
		@perl ${CORE_ROOT}/tools/NVRAMStatistic/nvram_lid_check.pl ${CORE_ROOT}/tools/NVRAMStatistic/lid/$(PROJECT_NAME)/$(FLAVOR) $(strip $(PROJDIR))/nvram_auto_gen $(strip $(COMPLOGDIR))/nvram_lid_check.log /s
...
ifneq ($(PCIBT_ACTION_LIST),)
	...
	@$(IF_EXIST) $(MODISBUILD_TST_DB)/MDDB.P_InfoCustomAppSrc.EDB $(THEN) ($(DEL_FILE) $(MODISBUILD_TST_DB)/MDDB.P_InfoCustomAppSrc.EDB) $(CMD_END)
...
ifneq ($(strip $(CLEAN_CODEGEN_DEPENDENCY)),)
	...
	@$(IF_NOT_EXIST) $(MODISBUILD_TST_DB)/pcore_db $(THEN) \
		$(MKDIR) $(MODISBUILD_TST_DB)/pcore_db $(CMD_END)
...
ifeq ($(strip $(UNIVERSAL_DATABASE_SUPPORT)),TRUE)
	@$(MAKE) -fmake/$(CORE_ROOT)/build.mak -r -R --no-print-directory GAS_FOLDER=$(strip $(CORE_ROOT))/modem/gas UAS_FOLDER=$(strip $(CORE_ROOT))/modem/uas XGEN_SWITCH_FDD_TDD=FDD $(strip $(TMPDIR))/~inc_fdd_def.tmp
	@$(MAKE) -fmake/$(CORE_ROOT)/build.mak -r -R --no-print-directory GAS_FOLDER=$(strip $(CORE_ROOT))/modem/gas_tdd128 UAS_FOLDER=$(strip $(CORE_ROOT))/modem/uas_tdd128 XGEN_SWITCH_FDD_TDD=TDD $(strip $(TMPDIR))/~inc_tdd_def.tmp
endif
...
$(strip $(TMPDIR))/~inc_def_no_inc.tmp: FORCE
	...
	@$(ECHOLONGSTRING2) $(strip $(TMPDIR))/~inc_def_no_inc.tmp -D $(COM_DEF_PATH)
...
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
  ...
  ifeq ($(strip $(MODIS_COMPILER)),VC2008)
	...
	@$(ECHOLONGSTRING2) $(strip $(TMPDIR))/~inc_fdd_def.tmp -D $(foreach mod,$(filter-out uas_tdd128 gas_tdd128, $(COMPLIST)),$(strip $(MODULEINFODIR))/$(mod)/$(mod).def)
...
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
  ...
  ifeq ($(strip $(MODIS_COMPILER)),VC2008)
	...
	@$(ECHOLONGSTRING2) $(strip $(TMPDIR))/~inc_tdd_def.tmp -D $(foreach mod,$(filter-out uas gas, $(COMPLIST)),$(strip $(MODULEINFODIR))/$(mod)/$(mod).def)
...
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
  ...
  ifeq ($(strip $(MODIS_COMPILER)),VC2008)
	...
	@perl ${CORE_ROOT}/tools/pack_dep_gen.pl $@ $@ $(patsubst %/,%,$(dir $@)) "$(basename $(notdir $@))\.log"
  else ifeq ($(strip $(MODIS_COMPILER)),MINGW)
...
# MoDIS should generate another one without include path in Target's folder
	@$(IF_NOT_EXIST) $(strip $(TMPDIR))/~inc_def_no_inc.tmp $(THEN) \
		$(MAKE) -fmake/${CORE_ROOT}/build.mak -r -R --no-print-directory $(strip $(TMPDIR))/~inc_def_no_inc.tmp $(CMD_END)
  ifeq ($(strip $(COMPILER)),GCC)
...
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
  ...
  ifeq ($(strip $(UNIVERSAL_DATABASE_SUPPORT)),TRUE)
	@echo $(CGEN) -c $(MODISBUILD_TST_DB)/msglog_db/libParseDbModem_fdd.db $(strip $(TSTDIR))/database/$(CGEN_DEF_FILE) $(strip $(TSTDIR))/database/Pc_Cnf $(CODEGEN_DATABASE_OUT) $(MODISBUILD_TST_DB)/enumFile >>$(CGEN_CFG_MODEM)
  else
...
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
  ...
  ifeq ($(strip $(UNIVERSAL_DATABASE_SUPPORT)),TRUE)
    ifeq ($(strip $(NO_LTECGEN)),TRUE)
		@echo $(CGEN) -ap5 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/libParseDbModem_tdd.db $(MODISBUILD_TST_DB)/enumFile MoDIS $(strip $(VERNO))  -qModSim-$(BUILD_TSTDIR)/src/tst_querysimid_modis.c>>$(CGEN_CFG_MODEM)
    else
		@echo $(CGEN) -ap5 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN_SEC) $(MODISBUILD_TST_DB)/msglog_db/libParseDbModem_fdd.db $(MODISBUILD_TST_DB)/enumFile >>$(CGEN_CFG_MODEM)
		@echo $(CGEN) -ap6 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/libParseDbModem_tdd.db $(MODISBUILD_TST_DB)/enumFile MoDIS $(strip $(VERNO))  -qModSim-$(BUILD_TSTDIR)/src/tst_querysimid_modis.c>>$(CGEN_CFG_MODEM)
    endif
  else
...
$(MODIS_RULESDIR)/codegen_dep/cgen_un.det: $(NEED_CHECK_DEPEND_LIST) $(MODIS_RULESDIR)/codegen_dep/cgen_ap_libParseDbModem.det
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_S,$@,T," >> $(MBIS_BUILD_TIME_TMP)
endif
...
ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
  ifneq ($(strip $(MODIS_CONFIG)),TRUE)
    ...
    ifeq ($(strip $(UNIVERSAL_DATABASE_SUPPORT)),TRUE)
	@echo $(CGEN) -cm5 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/custom_parse_db_fdd.db $(MODISBUILD_TST_DB)/enumFileCustom >>$(CGEN_CFG_MODEM)
	@echo $(CGEN) -cm  $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/custom_parse_db_tdd.db $(MODISBUILD_TST_DB)/enumFileCustom MoDIS $(strip $(VERNO)) >>$(CGEN_CFG_MODEM)
    else
  ...
  ifneq ($(strip $(MODIS_CONFIG)),TRUE)
    ...
    ifeq ($(strip $(UNIVERSAL_DATABASE_SUPPORT)),TRUE)
	@echo $(CGEN) -cm5 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/custom_parse_db_fdd.db $(MODISBUILD_TST_DB)/enumFileCustomSrc >>$(CGEN_CFG_MODEM)
	@echo $(CGEN) -cm  $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/custom_parse_db_tdd.db $(MODISBUILD_TST_DB)/enumFileCustomSrc MoDIS $(strip $(VERNO)) >>$(CGEN_CFG_MODEM)
    else
  ...
  ifneq ($(strip $(MODIS_CONFIG)),TRUE)
    ...
    ifeq ($(strip $(UNIVERSAL_DATABASE_SUPPORT)),TRUE)
	@echo $(CGEN) -cm5 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/custom_parse_db_fdd.db $(MODISBUILD_TST_DB)/enumFileCustomAppSrc >>$(CGEN_CFG_MODEM)
	@echo $(CGEN) -cm  $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/custom_parse_db_tdd.db $(MODISBUILD_TST_DB)/enumFileCustomAppSrc MoDIS $(strip $(VERNO)) $(MMIDIR) >>$(CGEN_CFG_MODEM)
    else
...
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
  ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
    ifeq ($(strip $(UNIVERSAL_DATABASE_SUPPORT)),TRUE)
	@echo $(CGEN) -ap7 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/srcParseDbModem_fdd.db $(MODISBUILD_TST_DB)/enumFileCustomApp >>$(CGEN_CFG_MODEM)
	@echo $(CGEN) -ap $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/srcParseDbModem_tdd.db $(MODISBUILD_TST_DB)/enumFileCustomApp $(strip $(PLATFORM))_$(strip $(CHIP_VER)) $(strip $(VERNO)) >>$(CGEN_CFG_MODEM)
    else
    ...
    ifeq ($(strip $(MTK_SUBSIDIARY)),TRUE)
      ifeq ($(strip $(UNIVERSAL_DATABASE_SUPPORT)),TRUE)
	@echo $(CGEN) -ap7 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/srcParseDbModem_fdd.db $(MODISBUILD_TST_DB)/enumFileCustomAppSrc >>$(CGEN_CFG_MODEM)
	@echo $(CGEN) -ap $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/srcParseDbModem_tdd.db $(MODISBUILD_TST_DB)/enumFileCustomAppSrc $(strip $(PLATFORM))_$(strip $(CHIP_VER)) $(strip $(VERNO)) >>$(CGEN_CFG_MODEM)
      else
  ...
  ifeq ($(strip $(CUSTOM_RELEASE)),FALSE)
    ifeq ($(strip $(UNIVERSAL_DATABASE_SUPPORT)),TRUE)
	@echo $(CGEN) -ap7 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/srcParseDbModem_fdd.db $(MODISBUILD_TST_DB)/enumFileCustomApp >>$(CGEN_CFG_MODEM)
	@echo $(CGEN) -ap  $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/srcParseDbModem_tdd.db $(MODISBUILD_TST_DB)/enumFileCustomApp MoDIS $(strip $(VERNO)) >>$(CGEN_CFG_MODEM)
    else
    ...
    ifeq ($(strip $(MTK_SUBSIDIARY)),TRUE)
      ifeq ($(strip $(UNIVERSAL_DATABASE_SUPPORT)),TRUE)
	@echo $(CGEN) -ap7 $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/srcParseDbModem_fdd.db $(MODISBUILD_TST_DB)/enumFileCustomAppSrc >>$(CGEN_CFG_MODEM)
	@echo $(CGEN) -ap  $(CODEGEN_DATABASE_OUT) $(CODEGEN_DATABASE_IN) $(MODISBUILD_TST_DB)/msglog_db/srcParseDbModem_tdd.db $(MODISBUILD_TST_DB)/enumFileCustomAppSrc MoDIS $(strip $(VERNO)) >>$(CGEN_CFG_MODEM)
      else
...
ifneq ($(strip $(MODIS_CONFIG)),TRUE)
	...
	@echo $(POSTGEN) -extract_all_em_info $(CODEGEN_DATABASE_OUT) $(MODISBUILD_TST_DB)/mdm_layout_desc.dat >>$(POSTGEN_CFG_MODEM)
	...
	@echo $(POSTGEN) -filter_gen bin $(CODEGEN_DATABASE_OUT) $(BUILD_TST_DB)/log2SD_Catcher_filters.ini $(BUILD_TST_DB)/pcore_db/catcher_P_filter.bin >>$(POSTGEN_CFG_MODEM)
	...
	@echo $(BUILD_TST_DB)/pcore_db/catcher_P_filter.bin >>$(CODE_GEN_LST)
...
# merge MD & DSP logging filter into merge_filter.bin
	@echo $(POSTGEN) -filter_merge $(CODEGEN_DATABASE_OUT) $(BUILD_TST_DB)/pcore_db/catcher_P_filter.bin $(BUILD_TST_DB)/dsp_filter.bin $(BUILD_TST_DB)/pcore_db/catcher_P_filter.bin>>$(POSTGEN_CFG_MODEM)
...
ifeq ($(wildcard common/service/dhl/database/nvram_db_cnf.ini),)
	...
	@$(ECHO) -n "$(POSTGEN) -meta_db_gen $(MODISBUILD_TST_DB)/$(BPMDMETADATABASE) $(CODEGEN_DATABASE_OUT) $(MODISBUILD_TST_DB)/nvram_db_cnf.ini -saveXmlFile-$(MODISBUILD_TST_DB)/$(BPMDMETADATABASEXML)" >>$(POSTGEN_CFG_MODEM)
else
	@$(ECHO) -n "$(POSTGEN) -meta_db_gen $(MODISBUILD_TST_DB)/$(BPMDMETADATABASE) $(CODEGEN_DATABASE_OUT) common/service/dhl//database/nvram_db_cnf.ini -saveXmlFile-$(MODISBUILD_TST_DB)/$(BPMDMETADATABASEXML)" >>$(POSTGEN_CFG_MODEM)
endif
ifdef COMPRESS_GEN
  ifeq ($(strip $(COMPRESS_GEN)),TRUE)
	@$(ECHO) -n "$(POSTGEN) -compress_gen $(MODISBUILD_TST_DB)/$(BPMDMETADATABASEXML).GZ $(MODISBUILD_TST_DB)/$(BPMDMETADATABASEXML)" >>$(POSTGEN_CFG_MODEM)
  endif
endif
else
...
# *************************************************************************
# TARGET: merge catcher filter bin
# *************************************************************************
ifeq ($(TEST_LOAD_TYPE),L1S)
  PCORE_CODEGEN_DATABASE_MDDB_OUT = $(strip $(MODISBUILD_TST_DB))/pcore_db/MDDB.P.EDB
  L1CORE_CODEGEN_DATABASE_MDDB_OUT := $(subst dhl/database,l1core/dhl/database,$(strip $(MODISBUILD_TST_DB)))/MDDB.L.EDB
else
  PCORE_CODEGEN_DATABASE_MDDB_OUT = $(strip $(CODEGEN_DATABASE_OUT))
  ifeq ($(CUSTOM_RELEASE),FALSE)
  L1CORE_CODEGEN_DATABASE_MDDB_OUT := $(subst dhl/database,l1core/dhl/database,$(strip $(MODISBUILD_TST_DB))/MDDB.L_$(strip $(PLATFORM))_$(strip $(CHIP_VER))_$(strip $(PURE_VERNO)).EDB)
  else
    L1CORE_CODEGEN_DATABASE_MDDB_OUT := $(subst dhl/database,l1core/dhl/database,$(strip $(MODISBUILD_TST_DB))/$(BPLGUINFOCUSTOMSRCP))
    L1CORE_CODEGEN_DATABASE_MDDB_OUT := $(subst .P_,.L_,$(strip $(L1CORE_CODEGEN_DATABASE_MDDB_OUT)))
  endif
endif

MERGED_CODEGEN_DATABASE_MDDB_OUT := $(subst pcore_db/,,$(subst .P,,$(PCORE_CODEGEN_DATABASE_MDDB_OUT)))
MDB_CNF := $(strip $(MODISBUILD_TST_DB))/MDBCnf.ini
MERGE_DB_LOG = $(strip $(COMPLOGDIR))/merge_db.log

MERGED_CATCHER_FILTER_OUT := $(strip $(MODISBUILD_TST_DB))/catcher_filter.bin
PCORE_CATCHER_FILTER_OUT  := $(strip $(BUILD_TST_DB))/pcore_db/catcher_P_filter.bin
L1CORE_CATCHER_FILTER_OUT := $(subst dhl/database,l1core/dhl/database,$(strip $(BUILD_TST_DB)))/catcher_L_filter.bin

ifneq ($(TEST_LOAD_TYPE),L1S)
merge_catcher: $(PCORE_CATCHER_FILTER_OUT) $(L1CORE_CATCHER_FILTER_OUT)
endif
merge_catcher: check_merge_db $(MDB_CNF)
merge_catcher:
ifneq ($(TEST_LOAD_TYPE),L1S)
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_S,$@,T," >> $(MBIS_BUILD_TIME_TMP)
endif
	@$(ECHO) -n "1st Merge DB into $(MERGED_CODEGEN_DATABASE_MDDB_OUT)"
	@$(CGEN) -ltedbv2 -merge $(MERGED_CODEGEN_DATABASE_MDDB_OUT) $(MDB_CNF)
	@$(ECHO) -n "Merge catcher filter into $(MERGED_CATCHER_FILTER_OUT)"
	@$(POSTGEN) -ltedbv2 -merge_multi_filters $(MERGED_CODEGEN_DATABASE_MDDB_OUT) $(PCORE_CATCHER_FILTER_OUT)*$(L1CORE_CATCHER_FILTER_OUT) $(MERGED_CATCHER_FILTER_OUT)
ifneq ($(strip $(NO_DSP)),TRUE)
ifneq ($(strip $(LOG2SD_CATCHER_FILTERS_LIST)),)
	@$(FOR) $(DECLARE_VAR)i $(IN) $(LOG2SD_CATCHER_FILTERS_LIST) $(DO) \
		($(IF_EXIST) $(MODISBUILD_TST_DB)/pcore_db/catcher_P_filter_$(PRE_VAR)i.bin $(THEN) \
			$(IF_EXIST) $(strip $(L1CORE_BUILD_TST_DB))/catcher_L_filter_$(PRE_VAR)i.bin $(THEN) \
				($(POSTGEN) -ltedbv2 -merge_multi_filters $(MERGED_CODEGEN_DATABASE_MDDB_OUT) $(MODISBUILD_TST_DB)/pcore_db/catcher_P_filter_$(PRE_VAR)i.bin*$(strip $(L1CORE_BUILD_TST_DB))/catcher_L_filter_$(PRE_VAR)i.bin $(MODISBUILD_TST_DB)/catcher_filter_$(PRE_VAR)i.bin) \
			$(CMD_END) \
		$(CMD_END)) \
	$(DONE)
endif
endif
	@$(IF_EXIST) $(MERGED_CATCHER_FILTER_OUT) $(THEN) ($(COPY_FILE) $(MERGED_CATCHER_FILTER_OUT) $(strip $(MODISBUILD_TST_DB))/catcher_filter__Default.bin) $(CMD_END)
	@$(IF_EXIST) $(MERGED_CODEGEN_DATABASE_MDDB_OUT) $(THEN) ($(DEL_FILE) $(MERGED_CODEGEN_DATABASE_MDDB_OUT)) $(CMD_END)
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_E,$@,T," >> $(MBIS_BUILD_TIME_TMP)
endif
endif
...
# TARGET: merge_db
# *************************************************************************
merge_db: check_merge_db $(MDB_CNF)
merge_db: $(PCORE_CODEGEN_DATABASE_MDDB_OUT) $(L1CORE_CODEGEN_DATABASE_MDDB_OUT)
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_S,$@,T," >> $(MBIS_BUILD_TIME_TMP)
endif
	@$(ECHO) -n "Final Merge DB into $(MERGED_CODEGEN_DATABASE_MDDB_OUT)"
	@$(CGEN) -ltedbv2 -merge $(MERGED_CODEGEN_DATABASE_MDDB_OUT) $(MDB_CNF)

	@$(IF_NOT_EXIST) $(strip $(MODISBUILD_TST_DB))/pcore_db $(THEN) $(MKDIR) $(strip $(MODISBUILD_TST_DB))/pcore_db $(CMD_END)

ifeq ($(wildcard $(CUSTOMIZATION)/log2SD_Catcher_filters_meta.ini),)
ifneq ($(TEST_LOAD_TYPE),L1S)
	@$(ECHO) -n "Merge catcher filter into $(MERGED_CATCHER_FILTER_OUT)"
	$(POSTGEN) -ltedbv2 -merge_multi_filters $(MERGED_CODEGEN_DATABASE_MDDB_OUT) $(PCORE_CATCHER_FILTER_OUT)*$(L1CORE_CATCHER_FILTER_OUT) $(MERGED_CATCHER_FILTER_OUT)
ifneq ($(strip $(NO_DSP)),TRUE)
...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(ECHO) -n "Generating MDBCnf.ini ..."
	@$(IF_EXIST) $(MERGE_DB_LOG) $(THEN) ($(DEL_FILE) $(MERGE_DB_LOG)) $(CMD_END)
	@$(IF_NOT_EXIST) $(strip $(L1CORE_CODEGEN_DATABASE_MDDB_OUT)) $(THEN) \
		(echo "\"$(L1CORE_CODEGEN_DATABASE_MDDB_OUT)\" does not exist.") $(AND) \
		(echo "Please do full build first.") $(AND) \
		(echo "Error: \"$(L1CORE_CODEGEN_DATABASE_MDDB_OUT)\" does not exist." >> $(MERGE_DB_LOG) ) $(AND) \
		(echo "Please do full build first." >> $(MERGE_DB_LOG) ) $(AND) \
		(exit 1) \
	$(CMD_END)  

	@$(IF_NOT_EXIST) $(strip $(PCORE_CODEGEN_DATABASE_MDDB_OUT)) $(THEN) \
		(echo "\"$(PCORE_CODEGEN_DATABASE_MDDB_OUT)\" does not exist.") $(AND) \
		(echo "Please do full build first.") $(AND) \
		(echo "Error: \"$(PCORE_CODEGEN_DATABASE_MDDB_OUT)\" does not exist." >> $(MERGE_DB_LOG)) $(AND) \
		(echo "Please do full build first." >> $(MERGE_DB_LOG)) $(AND) \
		(exit 1) \
	$(CMD_END)  
	@$(ECHO) -n $(PCORE_CODEGEN_DATABASE_MDDB_OUT) > $(MDB_CNF)
	@$(ECHO) $(L1CORE_CODEGEN_DATABASE_MDDB_OUT) >> $(MDB_CNF)
	@$(ECHO) -n "Generating MDBCnf.ini Done"
...
# *************************************************************************
# TARGET: $(L1CORE_CODEGEN_DATABASE_MDDB_OUT)
# *************************************************************************
ifeq ($(strip $(ACTION)),codegen)
$(L1CORE_CODEGEN_DATABASE_MDDB_OUT): FORCE
endif
$(L1CORE_CODEGEN_DATABASE_MDDB_OUT):
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_S,$@,T," >> $(MBIS_BUILD_TIME_TMP)
endif
	@$(ECHO) -n "Generating L1core codegen db ..."
	@perl l1core/make.pl "$(PROJECT_NAME)($(FLAVOR)).mak" codegen
...
# *************************************************************************
# TARGET: check_merge_db
# *************************************************************************
MDB_CNF_L1CHK = $(strip $(MODISBUILD_TST_DB))/MDBCnf_L1Chk.ini
MERGE_DB_LOG = $(strip $(COMPLOGDIR))/merge_db.log

check_merge_db: $(MDB_CNF_L1CHK)
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_S,$@,T," >> $(MBIS_BUILD_TIME_TMP)
endif
	@$(ECHO) -n "Check L1 module name and modem type in different DB $(MDB_CNF_L1CHK)"
	$(POSTGEN)  -ltedbv2 -eL1_chk $(MDB_CNF_L1CHK)
	@$(DEL_FILE) $(MDB_CNF_L1CHK)
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_E,$@,T," >> $(MBIS_BUILD_TIME_TMP)
endif
...
CGEN_ENC_MODULE_KEY_SETTING := ./${CORE_ROOT}/service/dhl/database/access_control/module_k.csv
...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(IF_NOT_EXIST) $(strip $(CGEN_ENC_MODULE_KEY_SETTING)) $(THEN) \
		...
		(exit 1) \
	$(CMD_END)
#@$(ECHO) -n "$(CGEN) -enc $(CODEGEN_DATABASE_IN) $(CODEGEN_DATABASE_IN) $(CGEN_KEY_STORE_FOLDER)/$(strip $(KEY_VERNO)).bin $(strip $(KEY_VERNO)) ./${CORE_ROOT}/service/dhl/database/access_control/group.ini" >> $(CGEN_CFG_MODEM)
...
# *************************************************************************
# TARGET: $(MDB_CNF_L1CHK)
# *************************************************************************
$(MDB_CNF_L1CHK): FORCE
ifeq ($(strip $(MBIS_EN)),TRUE)
	@$(ECHO) -n -t "T_S,$@,T," >> $(MBIS_BUILD_TIME_TMP)
endif
	@$(ECHO) -n "Generating MDBCnf_L1Chk.ini ..."
	@$(IF_NOT_EXIST) $(strip $(L1CORE_CODEGEN_DATABASE_MDDB_OUT)) $(THEN) \
		(echo "\"$(L1CORE_CODEGEN_DATABASE_MDDB_OUT)\" does not exist.") $(AND) \
		(echo "Please do full build first.") $(AND) \
	       (echo "Error: \"$(L1CORE_CODEGEN_DATABASE_MDDB_OUT)\" does not exist." >> $(MERGE_DB_LOG) ) $(AND) \
	...
	@$(IF_NOT_EXIST) $(strip $(PCORE_CODEGEN_DATABASE_MDDB_OUT)) $(THEN) \
		...
		(exit 1) \
	$(CMD_END)  
# @mv $(PCORE_CODEGEN_DATABASE_MDDB_OUT) $(PCORE_CODEGEN_DATABASE_MDDB_OUT_TEMP)
	@$(ECHO) -n $(PCORE_CODEGEN_DATABASE_MDDB_OUT) > $(MDB_CNF_L1CHK)
	@$(ECHO) $(L1CORE_CODEGEN_DATABASE_MDDB_OUT) >> $(MDB_CNF_L1CHK)
	@$(ECHO) -n "Generating MDBCnf_L1Chk.ini Done"
  ...
  ifeq ($(LTE_SOURCE_BUILD),FALSE)
    ...
    ifeq ($(strip $(DHL_SUPPORT)),TRUE)
      ifeq ($(LTE_SOURCE_BUILD),TRUE)
		...
		@$(IF_EXIST) $(MODISBUILD_TST_DB)/$(BPMDMETADATABASEXML).GZ $(THEN) \
			$(IF_EXIST) $(MODISBUILD_TST_DB)/$(BPMDMETADATABASEXML) $(THEN) \
				$(DEL_FILE) "$(MODISBUILD_TST_DB)/$(BPMDMETADATABASEXML)" \
			$(CMD_END) \
		$(CMD_END)
...
ifeq ($(strip $(MODIS_CONFIG)),TRUE)
	@$(IF_EXIST) $(strip $(MODISBUILD_TST_DB))/pcore_db $(THEN)  \
		$(COPY_FILE) $(strip $(MODISBUILD_TST_DB))/pcore_db/* $(strip $(MODISBUILD_TST_DB))/ >$(DEV_NUL) \
	$(CMD_END)
	@echo $(subst pcore_db/,,$(CODEGEN_DATABASE_OUT)) >>$(CODE_GEN_LST)
else
...
ifeq ($(strip $(call Upper,$(MTK_INTERNAL))),TRUE)
  ifneq ($(strip $(TEST_LOAD_TYPE)),BASIC)
	...
	@$(IF_EXIST) $(BUILD_TST_DB)/catcher_P_filter.bin $(THEN)\
		(echo $(CGEN) -sign $(strip $(NET_PATH))/$(BUILD_TST_DB)/catcher_P_filter.bin) $(AND) \
		($(CGEN) -sign "$(strip $(NET_PATH))"/$(BUILD_TST_DB)/catcher_P_filter.bin) $(AND) \
		($(COPY_FILE) $(BUILD_TST_DB)/catcher_P_filter.bin* $(strip $(TARGDIR))) $(CMD_END)
endif
endif
# mbis time probe
ifeq ($(strip $(MBIS_EN)),TRUE)
...
ifneq ($(SINGLE_CORE),TRUE)
	...
	@$(IF_EXIST) $(FIXPATH)/${CORE_ROOT}/tools/copro_info_gen.pl $(THEN)\
		(perl $(FIXPATH)/${CORE_ROOT}/tools/copro_info_gen.pl $(FORCE_GEN) $(strip $(TMPDIR))/~copro_info.tmp "$(strip $(PROJECT_MAKEFILE))" $(strip $(INFOMAKELOG)) $(SINGLE_CORE) > $(strip $(COMPLOGDIR))/copro_info_gen.log 2>&1) $(WITH) \
...
ifeq ($(strip $(MBIS_EN)),TRUE)
	...
	@$(ECHO) "$@ START TIME=" >> $(strip $(TARGDIR))/build.log
	@perl ${CORE_ROOT}/tools/time.pl -n>>$(strip $(TARGDIR))/build.log
	@$(IF_EXIST) $(FIXPATH)/${CORE_ROOT}/tools/sys_auto_gen.pl $(THEN) \
		((perl $(FIXPATH)/${CORE_ROOT}/tools/sys_auto_gen.pl $(SCATTERFILE) $(BL_SCATTERFILE) "$(strip $(PROJECT_MAKEFILE))" $(strip $(BUILD_SYSDIR)) $(strip $(NEED_BUILD_BOOTLOADER)) $(strip $(INSIDE_MTK)) $(strip $(DUMMY_LIS)) $(strip $(INFOMAKELOG)) $(strip $(TMPDIR))/~copro_info.tmp $(LIBDIR_TEMP) $(strip $(CC))> $(strip $(COMPLOGDIR))/sysgen.log) \
		$(WITH) \
...
# mbis time probe
  ...
  ifneq ($(strip $(MODIS_CONFIG)),TRUE)
		@$(IF_EXIST) ${COMMON_ROOT}/service/config/src/hal/syscomp_config.c $(THEN)\
			...
			(perl ${COMMON_ROOT}/tools/sys_mem_gen/sys_mem_gen.pl $(strip $(TMPDIR))/~syscomp_config.o $(strip $(BUILD_SYSDIR)) $(MODIS_CONFIG) $(strip $(COMPLOGDIR)) $(INFOMAKELOG) "$(CROSS)">>$(strip $(COMPLOGDIR))/sys_mem_gen.log $(WITH) \
		...
		@$(IF_EXIST) ${COMMON_ROOT}/service/config/src/hal/syscomp_config.c $(THEN)\
			($(CC) $(VIA)$(strip $(TMPDIR))/~config_optInc.tmp -c ${COMMON_ROOT}/service/config/src/hal/syscomp_config.c -o $(strip $(TMPDIR))/~syscomp_config.o 2>$(strip $(MODISLOGDIR))/sys_mem_gen.log $(AND) \
			(perl ${COMMON_ROOT}/tools/sys_mem_gen/sys_mem_gen.pl $(strip $(TMPDIR))/~syscomp_config.o $(strip $(BUILD_SYSDIR)) $(MODIS_CONFIG) $(strip $(MODISLOGDIR)) $(INFOMAKELOG) "$(CROSS)">>$(strip $(MODISLOGDIR))/sys_mem_gen.log $(WITH) \
