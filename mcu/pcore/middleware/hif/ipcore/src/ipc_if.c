...
void ipc_pdn_info_notify(module_type src_mod_id, local_para_struct *local_para_ptr)
{
    ...
    IPC_GET_PROTOID_FROM_PDNID(src_mod_id, proto_idx);
...
kal_bool ipc_on_downlink_ra_proxy(kal_uint32 pdn_id, qbm_gpd **p_head, qbm_gpd **p_tail)
{
    kal_bool                    result = KAL_FALSE;
    qbm_gpd                    *curr_gpd, *next_gpd, *priv_gpd;
    kal_bool                    is_spd, end_of_list;
    kal_uint8                  *ip_packet = NULL, *ra_data_p;
    ...
    priv_gpd = next_gpd = NULL;
    for(curr_gpd = *p_head; curr_gpd && !end_of_list; curr_gpd = next_gpd)
    {
        is_spd = (QBM_DES_GET_PDT(curr_gpd) != 0);
        next_gpd = QBM_DES_GET_NEXT(curr_gpd);
        end_of_list = (curr_gpd == *p_tail);

        if(is_spd) {
            ip_packet = QBM_DES_GET_DATAPTR(curr_gpd);
            packet_len = QBM_DES_GET_DATALEN(curr_gpd);
            IPC_ASSERT(packet_len > 0);
            ...
            for (idx = 0; idx < packet_num; idx++) {
                ...
                if (!QBM_SPD_PIE_GET_IGR(p_spd_pie)) {
                    if (ipc_spd_chk_is_icmp6_ra(ip_packet, payload_len)) {
                        ipc_proxy_ipv6_ra_pool_push(pdn_id, NULL);  // Try to release old RA GPD first
                        curr_gpd = (qbm_gpd*)qbm_alloc_one(QBM_TYPE_NET_DL);

                        if (curr_gpd) {
                            ipc_set_gpd_datalen(curr_gpd, payload_len, (void**)&ra_data_p);
                            IPC_ASSERT(ra_data_p);
                            QBM_CACHE_INVALID(ip_packet, payload_len);
                            kal_mem_cpy(ra_data_p, ip_packet, payload_len);
                            QBM_CACHE_FLUSH(ra_data_p, payload_len);
                            qbm_cal_set_checksum(curr_gpd);

                            ipc_proxy_ipv6_ra_pool_push(pdn_id, curr_gpd);
                            result = KAL_TRUE;
                        }
                        else {
                            result = KAL_FALSE;
                        ...
                        hif_trace_error(IPC_ON_DOWNLINK_RA_RPOXY_SPD, pdn_id, curr_gpd);
                    }
            ...
            if(ipc_gpd_chk_is_icmp6_ra(curr_gpd)) {

                    hif_trace_error(IPC_ON_DOWNLINK_RA_RPOXY_GPD, pdn_id, curr_gpd);
                    ...
                    if(priv_gpd) {
                        ...
                        *p_head = end_of_list? NULL:next_gpd;
                    }
    ...
    hif_trace_error(IPC_ON_DOWNLINK_RA_RPOXY);
...
drop:
    ipc_on_downlink_ra_proxy(pdn_id, &p_head, &p_tail);   // Store RA for IPv6 here
            ...
            if (session) {
                ...
                if (session)
                {
                    ...
                    if (IPC_IP_TYPE_IPV6 == ip_type || IPC_IP_TYPE_MIXED == ip_type)
                    {
                        ...
                        ra_gpd = ipc_proxy_ipv6_ra_pool_pop(param->pdn_id);
                        if(ra_gpd) {
                            ipc_pkt_t pkt;
                            ...
                            pkt.head = ra_gpd;
                            pkt.tail = ra_gpd;
                            ipc_send_dl_pkt(&pkt, NULL, param->network_interface_id);
                        }
                    }
