...
fp_nvram_read_handler_t FT_ReadFrom_NVRAM             = FT_NvramSec_ReadNvramHandler;
fp_nvram_write_handler_t FT_WriteTo_NVRAM             = FT_NvramSec_WriteNvramHandler;
#else // #if defined(__NVRAM_SECRET_DATA__)
...
void FT_DispatchMessage(ilm_struct* ptrMsg)
{
...
#endif // #if !defined(__DHL_MODULE__)
    {
        ...
        switch (ft_header->ft_msg_id)
        {
             ...
             ********************************************/
#if !defined(__LTE_SM__)&&defined(__MTK_GL1_GSM__)
    ...
    else if( KAL_TRUE == FT_Nvram_IsModNvramRespnose(ptrMsg) )
    {
       switch (ptrMsg->msg_id)
       {
           ...
           case MSG_ID_NVRAM_WRITE_CNF:
           {
               ...
               /* Handle Cal Data Integrity check OP (Stop Record) */
               if( KAL_TRUE == FT_Misc_IsCalIntegrityNvramOp(msg_ptr->access_id, msg_ptr->file_idx) )
               {
                   FT_Misc_CalDataSyncBufferToNvramResponseHandler(msg_ptr);
                   break;
               }
    ...
    else if( (ptrMsg->src_mod_id == MOD_GL1TST) )
    {
...
#if defined(__LTE_RAT__)
    else if( MOD_ETSTM == ptrMsg->src_mod_id || MOD_EL1TST == ptrMsg->src_mod_id )
    {
        FT_Erf_ConfirmHandler(ptrMsg);
    }
#endif // #if defined(__LTE_RAT__)
    else if( MOD_MMRF == ptrMsg->src_mod_id )
    {
