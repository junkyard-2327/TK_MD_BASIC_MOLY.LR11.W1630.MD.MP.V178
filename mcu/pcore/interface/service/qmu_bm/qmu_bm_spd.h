...
/*! @brief define the maximal packet number in a single SPD (default:30).
     
    Ideally, MD handles 30 packets per millisecond. Add one single element for 
    MBIM NTH header.
*/
#define QBM_SPD_MAX_PKTNUM  (30)
...
#else
#define RXSPD_INFO_FLAG_BIT_SKIP ((kal_uint8)0x01)

#define QBM_RXSPD_SET_SKIP(_p) \
	(QBM_GET_RXSPD_PTR(_p)->rxspd_info |= RXSPD_INFO_FLAG_BIT_SKIP)
...
#define QBM_RXSPD_SET_RES(_p, _l) \
	(QBM_GET_RXSPD_PTR(_p)->reserv_len = _l)
