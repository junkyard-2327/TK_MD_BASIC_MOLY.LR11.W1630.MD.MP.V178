...
/****************************************************************************/
/*! @brief the portotype of peekb4free callback funtion

    @since UMOLY
*/
/****************************************************************************/
typedef void (*qbm_peekb4free_cbk_f) ( void* p_head, void* p_tail );
//#ifdef __LWA_SUPPORT__
typedef void (*qbm_free_pit_ent_one_cbk_f)(void *ptr, kal_uint32 idx);
//#endif
...
//#ifdef __LWA_SUPPORT__
kal_uint32 qbm_free_pit_ent_one(void *ptr, kal_uint32 idx);
#define QBM_FREE_PIT_ENT_ONE(ptr, idx) qbm_free_pit_ent_one(ptr, idx)

kal_uint32 qbm_free_pit_ent_mult(void *p_head, void *p_tail);
#define QBM_FREE_PIT_ENT_MULT(p_head, p_tail) qbm_free_pit_ent_mult(p_head, p_tail)
//#endif
...
/****************************************************************************/
/*! @brief register a callback function with qbm type.

    QBM provides a new service for buffer allocator to peek the GPD/SPD before 
    it is destroyed. User SHOULD register this callback function for a buffer
    type before it starts allocating. Moreover, QBM invokes this function in 
    qbmt_dest_q() under the protection of enhance mutex. So, this callback 
    function SHOULD be short and efficient. The coding rule under enhance 
    mutex MUST be followed in this function as well. 
    The input GPD/SPD pointer is the real entity. For safety, the contents 
    is READ-ONLY.

    @note For debugging, QBM double-checks PD contents during QC

    @param[in] type qbm queue type.
    @param[in] fp callback function.
    @since UMOLY
*/
/****************************************************************************/
extern void qbm_reg_cbk_peek_before_free( qbm_type type, 
    qbm_peekb4free_cbk_f fp );

//#ifdef __LWA_SUPPORT__
extern void qbm_reg_cbk_free_pit_ent_one(qbm_free_pit_ent_one_cbk_f fp);

extern void qbm_reg_cbk_free_pit_ent_mult(qbm_peekb4free_cbk_f fp);
//#endif
/****************************************************************************/
#endif /* QMU_BM_H */
