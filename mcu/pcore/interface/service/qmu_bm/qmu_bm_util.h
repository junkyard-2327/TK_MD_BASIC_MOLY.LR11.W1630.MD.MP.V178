...
/****************************************************************************/
/*! @brief qbmt_de_q_hif_ul_spd

    De-q RXSPD from CLDMA QMU. This function dequeues RXSPDs from input queue and 
    rewrite them to GPD format. The RXSPDs with SKIP bit are removed automatically before
    function return.

    According to the original HIF_UL design, the first 160bytes of data part will be invalided.

    The return number might be less than the real PD number from pp_new_head to
    pp_new_tail. This is because the RXSPDs with SKIP flag are removed in this APIs.

    @note In the case of returning number > 0, it's possible that pp_new_head equals to NULL.

    @param[in] pp_src_head a pointer to the head of original queue on QMU.
    @param[in] pp_src_tail a pointer to the tail of original queue on QMU.
    @param[out] pp_new_head a pointer to the head of de-queued buffer list
    @param[out] pp_new_tail a pointer to the tail of de-queued buffer list

    @return The number of de-queued PDs (please read the function description )
    @since UMOLY
*/
/****************************************************************************/
kal_uint32
qbmt_de_q_hif_ul_spd(
		void **pp_src_head, 
		void **pp_src_tail, 
		void **pp_new_head, 
		void **pp_new_tail
...
/****************************************************************************/
/*! @brief qbm_util_rxspd2gpd

    Transform RXSPDs to GPDs. This function is used for testing purpose. Data buffer
    on RXSPD is separated into segment by input length (gpd_allow_len). Redirected the
    data pointer in RxSPD to those segment and rewrite RxSPD to GPD.

    RxSPD with no buffer will be freed.

    @param[in] p_rxspd_head 	a pointer to a head of rxspd list.
    @param[in] p_rxspd_tail 		a pointer to a tail of rxspd list.
    @param[out] pp_gpd_head 	a pointer to a head of gpd list.
    @param[out] pp_gpd_tail 	a pointer to a head of gpd list.
    @param[in] gpd_allow_len 	the allow length per GPD.
    
    @return The number of PDs which are transformed.
    @since UMOLY
*/
/****************************************************************************/
kal_uint32
