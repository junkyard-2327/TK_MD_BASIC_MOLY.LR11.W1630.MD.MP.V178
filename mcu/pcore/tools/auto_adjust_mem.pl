...
if($LEVEL == 1) {
  ...
  if(-e "$AAPMC_TEMP_FOLDER/~ModemReservedSize_AutoConfig.tmp") {
    ...
    print "perl pcore/tools/ModemReservedSize_AutoConfig.pl $AAPMC_MAP_FILE $AAPMC_SYM_FILE $AAPMC_LINKER_SCRIPT $AAPMC_CUSTOM_FOLDER_PATH \"$AAPMC_MAKEFILE\" $AAPMC_TEMP_FOLDER/~ModemReservedSize_AutoConfig.tmp $AAPMC_LOG $AAPMC_INFOMAKE_LOG $AAPMC_COPRO_INFO_LOG> $AAPMC_LOG_FOLDER/ModemReservedSize_AutoConfig.log 2>&1\n" if($DEBUG);
    $out_str = "Calculating correct Modem Reserved Size value...";
    $result = system("perl pcore/tools/ModemReservedSize_AutoConfig.pl $AAPMC_MAP_FILE $AAPMC_SYM_FILE $AAPMC_LINKER_SCRIPT $AAPMC_CUSTOM_FOLDER_PATH \"$AAPMC_MAKEFILE\" $AAPMC_TEMP_FOLDER/~ModemReservedSize_AutoConfig.tmp $AAPMC_LOG $AAPMC_INFOMAKE_LOG $AAPMC_COPRO_INFO_LOG> $AAPMC_LOG_FOLDER/ModemReservedSize_AutoConfig.log 2>&1") ;
    ...
    print "perl pcore/tools/cksysdrv.pl $FLASH_CFG $FSUSAGE_LOG $NVRAMID_LOG $FEATURE_OVER $SYSDRV $NVRAM_PSEUDO $SYS_PATH $AAPMC_TEMP_FOLDER/~FS_LinkError.tmp $PROJECT_NAME $AAPMC_LOG> $AAPMC_LOG_FOLDER/FileSystemConfig.log 2>&1\n" if($DEBUG);
    $out_str = "Calculating correct File System value...";
    $result = system("perl pcore/tools/cksysdrv.pl $FLASH_CFG $FSUSAGE_LOG $NVRAMID_LOG $FEATURE_OVER $SYSDRV $NVRAM_PSEUDO $SYS_PATH $AAPMC_TEMP_FOLDER/~FS_LinkError.tmp $PROJECT_NAME $AAPMC_LOG >> $AAPMC_LOG_FOLDER/FileSystemConfig.log 2>&1") ;
  ...
  if(($result>>8) == ERR::NO_MODIFY) { # NO_MODIFY: recommand a value but not modify custom_featureConfig.h || scatter file is checkin by human
    ...
    $out_str .= "Failed.\nsome errors occured while executing pcore/tools/$check_scrip.pl. Please check $check_log\n";
  } else {
    $out_str .= "Failed.\nsome errors occured while executing pcore/tools/$check_scrip.pl. Please check $check_log\n";
  }
