...
# Constants
#****************************************************************************
my $PCORE_UPDATE_DSP_ADDRESS_VERNO     = " v0.04_MP5";
                                         # v0.04_MP5 , 2017/May/15, New Layout
...
#****************************************************************************
# Variable for configuation (config by local variable in each section)
#****************************************************************************
my $strPcoreSymbolName;
my $strL1coreSymbolName;
...
# (1) L1 DSP BIN
{
    $strPcoreSymbolName  = "g_dsp_bin_ptr";
    $strL1coreSymbolName = "dsp_bin";
    ...
    update_pcore_l1core($nl1coreDSPSectionAddr);
}
...
# (2) L1core sst base
{
    ...
    $strPcoreSymbolName  = "l1core_sst_addr";
    $strL1coreSymbolName = "";    

    # 1. parsing L1CORE SYM "first"
    SymFileParser::ParseSYM($L1CORE_SYM_FILE);
    my $l1coreSymbolRef = SymFileParser::GetSymbol();
    my $nl1coreDSPSectionAddr = hex(SymFileParser::GetLinkerSymbolAddress("SST_PROTECT", LinkerSymPostfix::Base, LinkerSymPrefix::None));

    pcoreUpdateDSPSection_die("[20.0] LCORE variable not exist", __FILE__, __LINE__) if( $nl1coreDSPSectionAddr == undef );
...
sub update_pcore_l1core
{
    my ($nl1coreDSPSectionAddr) = @_;
    ...
    # 3. get section info
    # NOTE: thre are **TWO**  ROM ()ROM & ROM_P) now!
    my $strPcoreSymSection = @{$pcoreSymbolRef->{$strPcoreSymbolName}}[Symbol::Region]; 
