...
{  
    ...
    ($nITCMIndex, $nDTCMIndex, $nBTCM1Index) = ($self->{ExeRegionLookUpTable}{L1CORE_DUMP_REGION__ITCM}, 
                        $self->{ExeRegionLookUpTable}{L1CORE_DUMP_REGION__DTCM},
                        $self->{ExeRegionLookUpTable}{L1CORE_DUMP_REGION__BTCM1});
    ($nDTCM_Size, $nBTCM1_Size) = (undef, undef);
    $bIDSeperated = 0;
    foreach (@{$self->{MEMORYInfo}})
    {
        if($_->[0] eq "L1CORE_ITCM")
        {
            $self->{ExeRegion}{$nITCMIndex}->[InfoIndex::MaxSize] = hex($_->[2]);
            $bIDSeperated = 1; # currently, no merged BTCM
        }
        elsif($_->[0] eq "L1CORE_DTCM")
        {
            $nDTCM_Size = hex($_->[2]);
        }
        elsif($_->[0] eq "L1CORE_BTCM1")
        {
            $nBTCM1_Size = hex($_->[2]);
        }
    }
    $self->{ExeRegion}{$nDTCMIndex}->[InfoIndex::MaxSize] = $nDTCM_Size if($bIDSeperated == 1);
    $self->{ExeRegion}{$nBTCM1Index}->[InfoIndex::MaxSize] = $nBTCM1_Size if(exists $self->{ExeRegionLookUpTable}{L1CORE_DUMP_REGION__BTCM1});
