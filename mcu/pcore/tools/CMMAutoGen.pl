...
# Constants
#****************************************************************************
my $CMMGEN_VERNO     = " u1.01";
                      #  u1.01 , remove pcore's dump folder
...
my $REG_BASE_H = "./pcore/driver/regbase/inc/";
my %REG_BASE_DEFS;
...
if ($ACTION == 0)
{
    ...
    {
        my $BuildInfoCMM = "$CMMDIR/P_BuildInfo.cmm";
        my $TemplateFolder = "./pcore/tools/DebuggingSuite/Misc/";
        my $template = &CommonUtil::GetFileContent($TemplateFolder."P_BuildInfoTemplate.cmm");
...
if ($ACTION == 1)
{
    ...
    {
        my $DumpCMM = "$CMMDIR/PCoreOnTargetMemoryDump.cmm";
        if(-e $DumpCMM)
        {
            my $template = &CommonUtil::GetFileContent($DumpCMM);
            my $regions = &gen_dump_address_size_template($PLATFORM);
            $template =~ s/\[CMMAUTOGEN_DUMP_PCORE_REGIONS\]/$regions/g;
            #my $l1core_regions = gen_l1core_dump_address_size_template($PLATFORM);
            #$template =~ s/\[CMMAUTOGEN_DUMP_L1CORE_REGIONS\]/$l1core_regions/g;
...
{  
    ...
    my $template;
        ...
        foreach my $item (@$DumpRegion_aref)
        {
            ...
            next if($item->[0] =~/l1core/i);
            print $item->[0].":".$item->[1].",".$item->[2].", $strEnd\n";
            $template .= "print \"PCORE::Dumping [$item->[0]]sys_mem_$strBase.bin\"\n"; 
...
sub gen_l1core_dump_address_size_template
{  
    my ($bb) = @_;
    my $template;
        print "------------L1CORE On Target Dump Region Info--------------\n";
        &LISInfo::Parse($L1CORE_LISFILE);
        my $DumpRegion_aref = LISInfo::GetCMMDumpRegionInfo($bb);
        foreach my $item (@$DumpRegion_aref)
        {
            my $strBase = $item->[1];
            my $strEnd = CommonUtil::Dec2Hex(hex($item->[1])+hex($item->[2])-0x1);
            print $item->[0].":".$item->[1].",".$item->[2].", $strEnd\n";
            $template .= "print \"L1CORE::Dumping [$item->[0]]sys_mem_$strBase.bin\"\n"; 
            $template .= "DATA.SAVE.BINARY \"\&store_folder\\l1core\\sys_mem_$strBase.bin\" $strBase--$strEnd\n";
        } 
    print "----------------------------------------------------\n";
