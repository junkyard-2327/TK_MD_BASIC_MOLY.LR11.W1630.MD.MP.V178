...
const kal_uint8* custom_get_gprs_dialup_connect_post_string(kal_uint8 act)
{
    ...
    {
        strcpy((kal_char*)temp_string, " 236800");
    }
    else
    {
    //#if defined(__REL8__) && defined(__UMTS_FDD_MODE__)
    #if defined(__REL8__)
        if (gmss_is_wcdma_mode_activated() == KAL_TRUE) 
        {
            strcpy((kal_char*)temp_string, " 42000000");
        }
        else
    #endif
        {
            strcpy((kal_char*)temp_string,  " 7200000");
        }
    }    
#else
    ...
    {
        //#if defined(__REL8__) && defined(__UMTS_FDD_MODE__)
        strcpy((kal_char*)temp_string, " 42000000");   //42Mbps
    }
    #endif
    ...
    if (gmss_is_wcdma_mode_activated() == KAL_TRUE) 
    {
        ...
        strcpy((kal_char*)temp_string,  " 7200000");   //7.2Mbps
    }
    else if (gmss_is_tdscdma_mode_activated() == KAL_TRUE)
    {
        //#elif defined(__HSDPA_SUPPORT__) && defined(__UMTS_TDD128_MODE__)
        strcpy((kal_char*)temp_string,  " 2624000");   //2.6Mbps
    }
    #endif
    if ((gmss_is_wcdma_mode_activated() == KAL_TRUE) ||
        (gmss_is_tdscdma_mode_activated() == KAL_TRUE) )
    {
        //#elif defined(__UMTS_FDD_MODE__) || defined(__UMTS_TDD128_MODE__)
        strcpy((kal_char*)temp_string,   " 384000");   //384kbps
    }
    else
    {
    #if defined(__EGPRS_MODE__)
        strcpy((kal_char*)temp_string,   " 473600");   //473.6kbps
    #else
        strcpy((kal_char*)temp_string,    " 80000");   //80kbps  
    #endif
    }
#endif
