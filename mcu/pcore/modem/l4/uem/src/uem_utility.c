...
/*****************************************************************************
 * FUNCTION
 *  uem_get_current_date_time
 * DESCRIPTION
 *  This function is to get the current date & time from CLIB.
 * PARAMETERS
 *  time_info     IN     pointer to time_info[6];
 * RETURNS
 *  void
 * GLOBALS AFFECTED
 *  void
 *****************************************************************************/
void uem_get_current_date_time(kal_uint8 *time_info)
{

#ifdef __MTK_TARGET__
    struct tm current_time;
    kal_mem_set((void *)&current_time, 0x00, sizeof(struct tm));

#ifdef __SUPPORT_CLIB_TIME__
    {
        time_t current_raw_time = time(NULL);
        (void)localtime_r(&current_raw_time, &current_time);
    }
#endif

    time_info[0] = current_time.tm_mday;      /* Day */
    time_info[1] = current_time.tm_hour;      /* Hour */
    time_info[2] = current_time.tm_min;      /* Min */
    time_info[3] = current_time.tm_sec;      /* Sec */

    /* tm_mon returns months since 1900 */
    time_info[4] = (kal_uint8)(current_time.tm_year % 100);      /* Year */

    /* tm_mon returns months since january (0 - 11) */
    time_info[5] = current_time.tm_mon + 1;      /* Mon */
#endif
...
void uem_send_msg_to_sat(kal_uint32 src_mod_id, msg_type msg_name, void *data_ptr)
{
    ...
    switch (msg_name)
    {
        ...
        case MSG_ID_SAT_CURRENT_TIME_CNF:
        {
            ...
            if (1 == l4c_get_dongle_mode())
            {
                uem_get_current_date_time(l_current_time_cnf_data->time_info);
            }
            else
            {
