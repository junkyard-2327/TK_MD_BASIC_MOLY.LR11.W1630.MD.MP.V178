...
BEGIN_TRACE_MAP(MOD_SMSAL)
   ...
   TRC_MSG(INFO_SCA_LENGTH, "SCA length = %d")
    ...
    /*for CB feature to avoid conflict in the lower layers while receiving th CB messages*/
    TRC_MSG(FUNC_SMSAL_CB_UPDATE_LOWER_LAYERS, "[CB FSM] enter func smsal_cb_update_lower_layers")
    TRC_MSG(INFO_PS1_STATE_ENABLE_CB, "[CB FSM] This is PS1 and the state is %d, so enable the CB on this")
    TRC_MSG(INFO_PS2_STATE_ENABLE_CB, "[CB FSM] This is PS2 and the state is %d, so enable the CB on this")
    TRC_MSG(INFO_PS1_STATE_DISABLE_CB,"[CB FSM] This is PS1 and the state is %d, so disable the CB on this")
    TRC_MSG(INFO_PS2_STATE_DISABLE_CB,"[CB FSM] This is PS2 and the state is %d, so disable the CB on this")
    TRC_MSG(UNKNOWN_FSM_STATE, "[CB FSM] Unknown FSM state")
    TRC_MSG(FUNC_SMSAL_UPDATE_CB_FSM, "[CB FSM] enter func smsal_update_cb_fsm")
    TRC_MSG(INFO_UPDATE_PS1, "[CB FSM] Inform PS1 about the FSM STATE Transition")
    TRC_MSG(INFO_UPDATE_PS2, "[CB FSM] Inform PS2 about the FSM STATE Transition")
    TRC_MSG(INFO_NO_TRANSITION, "[CB FSM] No state transition occured so do nothing")
    TRC_MSG(FUNC_SMSAL_CB_FSM_STATE_TRANSIT, "[CB FSM] enter func smsal_is_cb_fsm_state_transition_required")
    TRC_MSG(INFO_PLMN_NOT_SAME, "[CB FSM] PLMN not same on both the PS")
    TRC_MSG(INFO_INVALID_PLMN, "[CB FSM] INVALID PLMN so no need to change the FSM state ")
    TRC_MSG(INFO_RAT_NOT_SAME, "[CB FSM] RAT not same on both the PS")
    TRC_MSG(INFO_CB_ENABLED_STATE, "[CB FSM] CB not enabled on both the PS: for 1 = %d, for 2 = %d")
    TRC_MSG(INFO_SIM_INSERTED, "[CB FSM] SIM Inserted for the PS: for PS1 = %d, for PS2 = %d")
    TRC_MSG(INFO_FSM_STATE_CHANGE, "[CB FSM] FSM STATE changed from %d to %d")
    TRC_MSG(FUNC_SMSAL_REG_STATE_IND, "[CB FSM] enter func smsal_reg_state_ind_hdlr")
    TRC_MSG(INFO_DUPLICATE_REG_STATE, "[CB FSM]PLMN and RAT are not changed so discard this indication")
    TRC_MSG(INFO_TRIGGER_PLMN_RAT, "[CB FSM] [PLMN/RAT change] Trigger to change the FSM state.")
    TRC_MSG(INFO_TRIGGER_CSCB, "[CB FSM] [CB ENABLE/DISABLE by AP.]Trigger to change the FSM state.")
    TRC_MSG(INFO_TRIGGER_BOOT, "[CB FSM] [CB ENABLE/DISABLE during boot up.]Trigger to change the FSM state during boot up.")
    TRC_MSG(FUNC_GET_FSM_STATE, "[CB FSM] enter func smsal_get_current_fsm_state current_fsm_state = %d")
    TRC_MSG(FUNC_SET_FSM_STATE, "[CB FSM] enter func smsal_set_current_fsm_state FSM STATE Updated to %d")
