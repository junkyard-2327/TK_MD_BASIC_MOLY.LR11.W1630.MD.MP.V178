 ...
 * $Log$
 *
 * 03 01 2017 srinivasa.gundurao
 * [MOLY00229341] Guaranteed Response Timer Interface Changes.
 * 	
 * Guaranteed PDN response Time, Interface Changes for LR11 MP5.
 *
 * 01 26 2016 tc.chang
 * [MOLY00162756] Follow VzW requirement if the requested APN is listed on the VzW APN table.
 *
 * 10 22 2015 jeffery.chen
 * [MOLY00144447] Operator Feature/Requirement
 * [MOLY00141163] [MT6290][VZW] Router Advertisement initial error handling change
 * Rename start_t3402 flag in epsb_deact_req_struct
 *
...
typedef struct 
{
     ...
     *  - TRUE,  if PDN-CONN-REQ is sent with ATTACH-request procedure, the ATTACH is rejected by the Network, and the EMM use the ESM_EMM_GET_ESM_CAUSE_IND to carry PDN_CONNECTIVITY_REJECT to ESM
     *  - FALSE, other cases (ex: ESM local reject PDN-CONN-REQ for ATTACH because APN is this_is_an_invalid_apn, or standalone PDN activation reject, ... etc)
     *    - The current-plmn-id would be updated by nwsel_pam_eq_plmn_list_update_ind_struct . 
     */
    kal_bool is_valid_current_plmn_id ;

    /** Originally sent by EMM to indicate the current-plmn-id for the ATTACH-reject. 
     *  - For 4G attach-reject case, there is no eq-plmn so only current-plmn-id is needed for PAM's arbitration . 
     */
