...
typedef struct{
    ...
    /* GAS sets this flag with TRUE when only neighbor cell info is updated and
         no other cell info changed from previous sys_info */
    kal_bool is_only_neighbor_cell_info_updated;
	/* neighbor cell info */
...
#ifdef __GEMINI_WCDMA_WORLD_PHONE__
	kal_bool is_gmss_list;	//List PLMN's with suitable cells
#endif
...
typedef struct{
    ...
    /**< RAL/MRU list */
    cell_band_info_struct list_info [SYS_MAX_MRU_LIST_NUM];
...
typedef struct {
    LOCAL_PARA_HDR
    kal_bool is_gemini_ps_high_sem_req;
} ratcm_gas_ps_session_stop_req_struct;
#endif /* __MULTIPLE_PS__ */
...
/* Universal Bin duplex mode switch */
typedef struct{
    LOCAL_PARA_HDR
...
} ratcm_gas_duplex_mode_change_req_struct;

typedef struct{
    LOCAL_PARA_HDR
    kal_bool result;
} ratcm_gas_duplex_mode_change_cnf_struct;
...
}ratcm_gas_serving_cell_signal_quality_ind_struct;
#endif
