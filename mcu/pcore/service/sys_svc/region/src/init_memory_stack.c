...
#if defined(__HIF_CCCI_SUPPORT__) && defined(__PCORE__)
                                 + 1
#if defined(__LOG_BUF_IN_SHM_SUPPORT__) || defined(__LWA_SUPPORT__)
                                 + 1
#endif
#endif
...
static kal_uint32 SlaveSysMemoryInfo[(L1CORE_DUMP_REGION_COUNT)* 2];
...
/* under construction !*/
#else /* for TX is ZI */
    while (region_sz < ((kal_uint32)&Image$$EXTSRAM_DSP_TX$$ZI$$Length
...
/* under construction !*/
#else /* for RX is ZI */
    while (region_sz < ((kal_uint32)&Image$$EXTSRAM_DSP_RX$$ZI$$Length
...
void INT_GetSysMemoryInfo( kal_uint32 **info, kal_uint16* count )
{
   ...
   SysMemoryInfo[var_index++] = share_startaddr;
   SysMemoryInfo[var_index++] = share_len;   
#endif  /* __LOG_BUF_IN_SHM_SUPPORT__ */
#endif  /* __HIF_CCCI_SUPPORT__ */
...
void INT_GetSlaveSysMemoryInfo( kal_uint32 **info, kal_uint16* count )
{
   kal_uint32 var_index = 0, count_sys_num = 0;
    ...
    /* get regions to be dumped from scatter struct */
    var_index = custom_query_l1core_dump_region((EXTSRAM_REGION_INFO_T*)SlaveSysMemoryInfo);

    /* calculate the number of regions to be dumped */
   *info  = (kal_uint32*)SlaveSysMemoryInfo;
   *count = var_index / 2;
   count_sys_num = sizeof(SlaveSysMemoryInfo) / sizeof(kal_uint32) / 2;
   if (*count > count_sys_num) {
      ASSERT(0);
   }
}
...
#define GET_CURRENT_TIME(v) \
    do { \
        CP15_PMU_GET_CYCLE_CNT(v); \
    } while (0)

#define GET_DURATION(_d,_t1,_t2) \
    do { \
        _d = ((_t2) >= (_t1)) ? ((_t2) - (_t1)) : ((0xFFFFFFFF) - (_t1) + (_t2)); \
    } while (0)


kal_int32 l1gdma_copy(kal_uint8 *src, kal_uint8 *dst, kal_uint32 size)
{
    kal_int32 drv_ret = L1GDMA_RET_OK;
    l1gdma_cfg_t gdma_cfg;
    volatile kal_uint32 start_cycle = 0, current_cycle = 0, duration = 0;

    do {
        /*
         * Check if the channel is in idle state
         */
        if (drv_l1gdma_get_status(L1GDMA_CH_EX_COPY) != GDMA_STOP) {
            drv_ret = L1GDMA_RET_CHN_NOT_STOPPED;
            break;
        }
        /*
         * Setup configuration
         */
        kal_mem_set(&gdma_cfg, 0, sizeof(gdma_cfg_t));
        gdma_cfg.gdma_mod = GDMA_MOD_BASIC;
        gdma_cfg.gdma_sel_channel = L1GDMA_CH_EX_COPY;
        gdma_cfg.gdma_gpd_addr.gdma_src_Gpd = (kal_uint32)src;
        gdma_cfg.gdma_gpd_addr.gdma_dst_Gpd = (kal_uint32)dst;
        gdma_cfg.gdma_mod_basic_tx_size = size;
        gdma_cfg.gdma_bus_width.src_bus_width = GDMA_BUS_WIDTH_32BITS;
        gdma_cfg.gdma_bus_width.dst_bus_width = GDMA_BUS_WIDTH_32BITS;
        if (drv_l1gdma_set_config(&gdma_cfg) != KAL_TRUE) {
            drv_ret = L1GDMA_RET_SET_CONFIG_FAIL;
            break;
        }
        /*
         * Start command
         */
        if (drv_l1gdma_start_cmd(L1GDMA_CH_EX_COPY, GDMA_START) != KAL_TRUE) {
            drv_ret = L1GDMA_RET_START_FAIL;
            break;
        }
        /*
         * Pooling channel status. Set 1 second timeout.
         */
        GET_CURRENT_TIME(start_cycle);
        while(drv_l1gdma_get_status(L1GDMA_CH_EX_COPY) == GDMA_STARTED) {
            GET_CURRENT_TIME(current_cycle);
            GET_DURATION(duration, start_cycle, current_cycle);
            if ((1000000 * CPU_CLOCK_MHZ) < duration)
                break;
        };
    } while (0);
