...
void nvram_write_exception(kal_uint16 length, long *ex_data, long *ex_log)
{
    ...
    if (ex_log == NULL)
    {
        /* kal_print("nvram_write_exception(): ex_log == NULL\n"); */
        nvram_trace(TRACE_FUNC, FUNC_NVRAM_WRITE_EXCEPTION1);
        return;
    }
    ...
    */
    ...
    /* Save the current statistics */
    {
        ...
        nvram_read_data_item(ldi, 1, 1, (kal_uint8*) statistic, NVRAM_EF_SYS_STATISTICS_SIZE);
...
//#ifdef __NVRAM_PSEUDO_MERGE__
    nvram_trace(TRACE_FUNC, FUNC_NVRAM_GET_DEFAULT_VALUE, ldi->LID, result, ldi->size, ldi->fileprefix);
//#else
