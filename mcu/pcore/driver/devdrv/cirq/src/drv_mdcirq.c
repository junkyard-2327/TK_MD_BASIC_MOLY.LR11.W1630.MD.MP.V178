 ...
 * MT6757 call for check-in: PCORE MDCIRQ.
 *
 * 02 16 2016 i-chun.liu
 * [MOLY00164979] MT6757 Call for check-in
 * MT6757 call-for-check-in: PCORE MDCIRQ.
 *
 * 10 05 2015 i-chun.liu
 * [MOLY00139905] Modify Pcore Qbit checking to use USC
 * mdcirq dummy submit.
 *
 * 08 04 2015 i-chun.liu    
 * [MOLY00135138] MT6797 bring up call for check in
 * MT6797 Pcore MDCIRQ call for check in .
 *
...
#else
   /* Stephen 2014: copy from stub_misc.c
    *       MASE will write IRQ_SOFTL_CWR/IRQ_SOFTH_CWR to control i_SimulationCtrl object
    *       From reg_base_mt6290.h: #define BAES_ADDR_SIM_CTRL (0xE0500000)
    */
   #define IRQ_SOFTL_CWR    ((unsigned int *)(0xE0500008))
   #define IRQ_SOFTH_CWR    (IRQ_SOFTL_CWR)

   ASSERT(code < NUM_IRQ_SOURCES);
   if (code <= 31)
   {
      *IRQ_SOFTL_CWR |= (1 << code);
   }
   else
   {
      *IRQ_SOFTH_CWR |= (1 << (code - 32));
   }
#endif
   RestoreIRQ(savedMask);
}
