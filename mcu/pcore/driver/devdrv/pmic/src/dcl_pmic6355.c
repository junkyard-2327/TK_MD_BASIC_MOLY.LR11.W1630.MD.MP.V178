...
#if defined(DCL_PMIC_ACCESS_TIME_LOG)
    pmic_access_duration_log[PMIC_INTERFACE_DBG][pmic_access_duration_index].end_time = ust_get_current_time();
    pmic_access_duration_log[PMIC_INTERFACE_DBG][pmic_access_duration_index].duration_time = ust_get_duration(pmic_access_duration_log[PMIC_INTERFACE_DBG][pmic_access_duration_index].start_time,
																							 pmic_access_duration_log[PMIC_INTERFACE_DBG][pmic_access_duration_index].end_time);
#endif
...
//for mt6335,37	  dcl_pmic_byte_write_nolock(reg, val);
    ...
    switch(cmd)
    {
        case LDO_BUCK_SET_EN:	//Enable control in SW mode
        {
            ...
            switch(pLdoBuckCtrl->mod)
            {
				...
				case VPA_SW:
                {
                    dcl_pmic_field_write(PMIC_ENUM_RG_BUCK_VPA_EN, pLdoBuckCtrl->enable);
                    return_val = STATUS_OK;
                }
                break;
		...
		case LDO_BUCK_SET_MODESET:
		{
            ...
            switch(pLdoBuckCtrl->mod)
            {
                case VPA_SW:
                {
                    dcl_pmic_field_write(PMIC_ENUM_RG_VPA_MODESET, pLdoBuckCtrl->mode);
                    return_val = STATUS_OK;
                }
                break;
        ...
        case LDO_BUCK_SET_VOLTAGE:
        {
            ...
            switch(pLdoBuckCtrl->mod)
            {
                ...
                case VPA_SW:
                {
                    dcl_pmic_field_write(PMIC_ENUM_RG_BUCK_VPA_VOSEL, regVal);
                    return_val = STATUS_OK;
                }
                break;
