 ...
 * $Log$
 *
 * 08 13 2015 i-chun.liu
 * [MOLY00137227] [SystemService][DebuggingSuite][Request For Design Change] Check and show RXBRP bus hang message in exception
 * bus recorder dubug function update.
 *
...
/* Debug control */
//#define BUSMON_DEBUG
static kal_hisrid busmon_hisrid;
#ifdef BUSMON_DEBUG
typedef enum {
   BUSMON_MSG_VERBOSE,
   BUSMON_MSG_INFO,
...
#define busrec_base 0xF03C0000
#define busrec_W_CNT busrec_base+0x10
#define busrec_W_ID_START busrec_base+0x20
#define busrec_W_ADDR_START busrec_base+0x24
#define busrec_R_CNT busrec_base+0x210
#define busrec_R_ID_START busrec_base+0x220
#define busrec_R_ADDR_START busrec_base+0x224

kal_bool drv_busrec_check()
{
    kal_uint32 W_count, i;
    W_count = DRV_Reg32(busrec_W_CNT);
    if((W_count > 0) && (DRV_Reg32(busrec_base)==0x20141107) )
    {
         for(i=0;i<W_count;i++)
         {
              if( (DRV_Reg32(busrec_W_ID_START+(i*0x8))==0x380) || (DRV_Reg32(busrec_W_ID_START+(i*0x8))==0x384) )
              return KAL_TRUE;
         }
    }
    return KAL_FALSE;
...
void drv_busmon_lisr(kal_uint32 v)
{
   ...
   kal_activate_hisr(busmon_hisrid);
   #else
   if (busmon_int_cb)
      (*busmon_int_cb)();
   kal_activate_hisr(busmon_hisrid);
...
void drv_busmon_set_trg_cb(busmon_intr_cb callback)
{
   ...
   busmon_hisrid = kal_init_hisr(BUSMON_HISR);
...
void drv_aximon_dump_cfg(kal_uint32 base_addr, busmon_ip_t mon_ip)
{
   ...
   BUSMON_MSG(BUSMON_MSG_VERBOSE, "    AXIMON_TST=0x%x\r\n", val);
   ...
   BUSMON_MSG(BUSMON_MSG_VERBOSE, "    AXIMON_IP%d_TG=0x%x\r\n", mon_ip, val);
   ...
   BUSMON_MSG(BUSMON_MSG_VERBOSE, "    AXIMON_IP%d_ADDR=0x%x\r\n", mon_ip, val);

   val = DRV_Reg32(ip_cfg_base +  AXIMON_IP0_ADDRMSK);
   BUSMON_MSG(BUSMON_MSG_VERBOSE, "    AXIMON_IP%d_ADDRMSK=0x%x\r\n", mon_ip, val);

   val = DRV_Reg32(ip_cfg_base +  AXIMON_IP0_DATAL);
   BUSMON_MSG(BUSMON_MSG_VERBOSE, "    AXIMON_IP%d_DATAL=0x%x\r\n", mon_ip, val);

   val = DRV_Reg32(ip_cfg_base +  AXIMON_IP0_DATAH);
   BUSMON_MSG(BUSMON_MSG_VERBOSE, "    AXIMON_IP%d_DATAH=0x%x\r\n", mon_ip, val);

   val = DRV_Reg32(ip_cfg_base +  AXIMON_IP0_DATAMSKL);
   BUSMON_MSG(BUSMON_MSG_VERBOSE, "    AXIMON_IP%d_DATAMSKL=0x%x\r\n", mon_ip, val);

   val = DRV_Reg32(ip_cfg_base + AXIMON_IP0_DATAMSKH);
   BUSMON_MSG(BUSMON_MSG_VERBOSE, "    AXIMON_IP%d_DATAMSKH=0x%x\r\n", mon_ip, val);
#endif /* BUSMON_DEBUG */
}
