 ...
 * $Log$
 *
 * 11 29 2016 shenghui.shi
 * [MOLY00215871] update UART driver to support MT6750S
 *
 * 02 02 2016 shenghui.shi
 * [MOLY00163868] [UART]Chip flavor update for Olympus
 * .
 *
 * 08 03 2015 yuke.ren
...
static kal_hisrid uart1_hisrid;
static kal_hisrid uart2_hisrid;
static kal_hisrid gdma2_hisrid;
   ...
   if(INT_QueryExceptionStatus())
	...
	switch(port)
	{
		case uart_port1:	
			...
			IRQUnmask(irq_code);
			uart1_hisrid = kal_init_hisr(UART_1_HISR);
			break;
		...
		case uart_port2:
			...
			DRV_WriteReg32(UART_IER(UART_BASE), 0x4);
			IRQUnmask(irq_code);
			uart2_hisrid = kal_init_hisr(UART_2_HISR);
			break;
		default:
			ASSERT(0);
			break;
	...
	IRQ_Register_LISR(MD_IRQID_MDGDMA2, uart_hdma_lisr, "UART_HDMA");
	IRQSensitivity(MD_IRQID_MDGDMA2, LEVEL_SENSITIVE);
	if (gdma2_hisrid == NULL) 
	  gdma2_hisrid = kal_init_hisr(GDMA2_HISR);
...
void UART1_LISR(kal_uint32 vector)
{
	...
	kal_activate_hisr(uart1_hisrid);
}
...
void UART2_LISR(kal_uint32 vector)
{
	...
	kal_activate_hisr(uart2_hisrid);
}
...
{
	IRQMask(MD_IRQID_MDGDMA2);
	kal_activate_hisr(gdma2_hisrid);
}
	...
	} else
	{
		if(en_q_or_de_q == (kal_bool) UART_EN_Q)
		{
			if(uart_rgpd_head[port] == NULL && uart_rgpd_tail[port] == NULL)
			{
				...
				HDMA_BUF0_PROG_ADDR_CONFIG((4 + port), p_ior_head);
...
)
{
    ...
    hdma_ch = (tx_or_rx == (kal_bool)UART_TX)?port+2:port+4;
