 ...
 * Description:
 * ------------
 *  This file implements the emi driver functions
 *
 * Author:
 * -------
 * -------
 ...
 * $Log$
 *
 * 01 17 2014 junhwa.kuo
 * [MOLY00054189] [MT6290] Update MT6290 EMI driver for PD reigster access
 * 	1. Update MT6290 EMI driver for power-down register access
 * 	2. update MT6290 EMI driver for profiling init function
 *
 * 01 10 2014 guo-huei.chang
 * [MOLY00052985] [Thermal] Support AT command for query AUXADC and DRAM temperature
 * 	add emi_get_dram_refresh_rate API
 *
 * 01 09 2014 guo-huei.chang
 * [MOLY00052985] [Thermal] Support AT command for query AUXADC and DRAM temperature
 * 	add get_dram_temp API
 *
 * 01 06 2014 guo-huei.chang
 * [MOLY00052585] [MT6290] Get DRAM MR4 Refresh Rate API
 * 	create API for get DRAM refresh rate
 *
 * 11 20 2013 walt.huang
 * [MOLY00047074] EMI driver supporting
 * *: Modify global variable to static.
 *
 * 11 19 2013 walt.huang
 * [MOLY00046927] [MakeFile] [Modify Common Makefile] Change EMI log from L1 trace to DHL trace
 * *: Update EMI code to use DHL trace.
 *
 * 10 07 2013 walt.huang
 * [MOLY00040184] [MakeFile] [Modify: Others] Add EMI trace log file
 * *: Fix MT6290M_SP(3G_EXT_SP).mak compiling error.
 *
 * 10 07 2013 walt.huang
 * [MOLY00040184] [MakeFile] [Modify: Others] Add EMI trace log file
 * *: Bypass EMI ELT trace log temporary for solving compiling error with some Makefiles.
 *
 * 10 07 2013 walt.huang
 * [MOLY00040184] [MakeFile] [Modify: Others] Add EMI trace log file
 * *: Fix compiler error due to some makefile will not run codegen.
 *
 * 10 07 2013 walt.huang
 * [MOLY00040184] [MakeFile] [Modify: Others] Add EMI trace log file
 * -: Remove bus_drv.h including.
 *
 * 10 07 2013 walt.huang
 * [MOLY00040184] [MakeFile] [Modify: Others] Add EMI trace log file
 * +: Add dummy API to avoid building error.
 *
 * 10 07 2013 walt.huang
 * [MOLY00040184] [MakeFile] [Modify: Others] Add EMI trace log file
 * *: Update EMI Profiling Interface.
 *
...
#define ROUND_TTL_NUM_LOG			9
#define ROUND_TTL_NUM				(1 << ROUND_TTL_NUM_LOG)
/* ==================== */
/*	INCLUDES			*/
/* ==================== */
#include "drv_comm.h"
...
kal_int32 emi_profiling_init(kal_uint32 ebm_rw) {
	...
	DRV_WriteReg32(REG_EMIARB_TESTD, DRV_Reg32(REG_EMIARB_TESTD) & 0xFFFFFCFF);
	DRV_WriteReg32(REG_EMIARB_TESTD, DRV_Reg32(REG_EMIARB_TESTD) | 0x00000000);

	// Set Overhead value according to pre-calculation (depend on BWL window)
	DRV_WriteReg32(REG_EMIARB_CONB, 0x42656FAD);
	DRV_WriteReg32(REG_EMIARB_CONC, 0x313E3857);
	DRV_WriteReg32(REG_EMIARB_COND, 0x42656FAD);
	DRV_WriteReg32(REG_EMIARB_CONE, 0x313E3857);

	// BWL Settings per Layer
	//DRV_WriteReg32(REG_EMIARB_ARBA, 0x00005829);
	//DRV_WriteReg32(REG_EMIARB_ARBB, 0x00005820);
	//DRV_WriteReg32(REG_EMIARB_ARBC, 0x00005820);
	//DRV_WriteReg32(REG_EMIARB_ARBD, 0x00005820);
	//DRV_WriteReg32(REG_EMIARB_ARBE, 0x00005808);
	//DRV_WriteReg32(REG_EMIARB_ARBF, 0x00005820);
