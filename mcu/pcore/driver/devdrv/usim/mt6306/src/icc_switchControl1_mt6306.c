...
PREFER_ORI:
	...
	*/
	...
	for(retry = 0; retry < ATR_RETRY; retry++)
	{
		...
		{
			dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "[%s]  interface:%d, retry %d, ori_ExpectVolt:%d\n\r", __FUNCTION__,hw_cb->simInterface, retry,ori_ExpectVolt);
...
// abnormal card
				...
				for(i=0x08;i<0x1E;i++)
				{
					if(i>0x13&&i<0x1A)continue;
					kal_uint8 readValue= sim_MT6306_SPIRead(switch_CB,i);		
					kal_sprintf(sim_dbg_str, "\r\n[%s] Interface:%d, %x:%x", __func__, hw_cb->simInterface, i, readValue);
					if(readValue!=0xFF)
						DRV_ICC_print_str(sim_dbg_str);
				}
				
				kal_sprintf(sim_dbg_str, "\r\n[%s] Interface:%d, GPIOMODE:%x, %x, DIR:%x", __func__, hw_cb->simInterface, (DRV_Reg32(0xC00053B0) >> 4)&0x7, (DRV_Reg32(0xC00053B0)>>8) & 0x7, (DRV_Reg32(0xC0005020) >> 25) & 0x03);
				DRV_ICC_print_str(sim_dbg_str);
