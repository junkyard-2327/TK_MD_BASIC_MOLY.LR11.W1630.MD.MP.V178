...
void sim_MT6306_SPIWrite(sim_MT6306_switchInfo *switch_CB, kal_uint16 data)
{
    #if !defined( SIM_DRV_GEMINI_WITH_MT6306 ) /*DSDS_4SIM will reset in HISR. We don't need to check this*/
	...
	else
	{
		...
		if(switch_CB->sim_MT6306_registeredLISR != card_cb->lisrState)
		{
			...
			else if(sim_MT6306_LISRUsim == card_cb->lisrState)
			{
				...
				{
					#if defined(__UNIFIED_ISR_LEVEL__)
					DRV_Register_HISR(DRV_USIM_HSIR_ID, usim_hisr_MT6306);
					#endif
				...
				{
					#if defined(__UNIFIED_ISR_LEVEL__)
					DRV_Register_HISR(DRV_USIM2_HSIR_ID, usim_hisr2_MT6306);
					#endif
...
void sim_MT6306_endOfAction(sim_HW_cb *hw_cb)
{
	...
	if(KAL_TRUE == card_cb->sim_MT6306_clkStopQueued)
	{
		...
		if(switch_CB->sim_MT6306_LPDisabled == (1<<hw_cb->simSwitchPortNo)) //Only this task entered
...
void sim_MT6306_clkStopper(sim_HW_cb *hw_cb)
{
    ...
    {   
        MT6306_DBG("[%s][ERR]: Interface:%d, card_cb->state:%d\r\n", __func__, hw_cb->simInterface, card_cb->state);
        ASSERT(0);
	}
    card_cb->sim_MT6306_clkStopQueued=KAL_FALSE;
...
void sim_MT6306_clkStopTimeoutHandler(void *parameter)
{
	...
	if(sim_MT6306_stateClkStopped == card_cb->state)
		return;
