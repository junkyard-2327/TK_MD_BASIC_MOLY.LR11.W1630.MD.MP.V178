...
/*******************************************************************************************/
	...
	{
		//dbg_print("PTS Fail!\r\n");
		kal_sprintf(sim_dbg_str, "[SIM_DRV:%d][ERR]PPS exchange fail\n\r",hw_cb->simInterface);
		DRV_ICC_print_str(sim_dbg_str);
...
static void usim_set_speed(usim_speed_enum speed, sim_HW_cb *hw_cb)
{
	...
	{		
		...
		kal_sprintf(sim_dbg_str, "[SIM_DRV:%d]Set Slow CLK speed!!!!\n\r",hw_cb->simInterface);
		DRV_ICC_print_str(sim_dbg_str);
...
#endif
		//DRV_ICC_print(4124, DRV_Reg32(0xC0005330), DRV_Reg32(0xC0005340), DRV_Reg32(0xC00020D0), 0 ,0);
...
//			DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC20, hw_cb->mtk_baseAddr, SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_ATIME_MTK), SIM_Reg32(0x80000450), hw_cb->mtk_pdnAddr, 0x1116);	
			DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC20, SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_ATIME_MTK), SIM_Reg32(0x80000450), 0, usim_dcb->ev_status, 0x1116);
	...
	switch(usim_dcb->main_state)
	{
		case (volatile usim_main_state_enum) ATR_STATE:
			...
			// read the remaining bytes of ATR
			{
				...
				USIM_SET_EVENT_Multiple(usim_dcb);
				kal_sprintf(sim_dbg_str,"[SIM_DRV:%d][ERR] ATR TOUT, usim_dcb->ev_status Status:%x\n\r",hw_cb->simInterface,usim_dcb->ev_status);
				DRV_ICC_print_str(sim_dbg_str);
			}
			break;
...
void usim_hisr(void)
{
	...
	kal_sprintf(sim_dbg_str, "[SIM_DRV:%d]:%x\n\r", hw_cb->simInterface, int_status);
	DRV_ICC_print_str(sim_dbg_str);
	...
	if(usim_dcb->previous_state == ACTIVATION_STATE || usim_dcb->previous_state == ATR_STATE)
	{
		...
		if(usim_dcb->atr_count > 40)
		{
			...
			usim_dcb->atr_count = 0;
                    ...
                    kal_sprintf(sim_dbg_str, "[SIM DRV:%d]SIM1 card send too many ATR data\n\r", hw_cb->simInterface);
	             DRV_ICC_print_str(sim_dbg_str);
			goto end_of_hisr;
		}
	} else
    ...
    if(int_status & SIM_STS_RXERR )
	{
        ...
        usim_set_timeout(0, hw_cb);
		usim_dcb->ev_status = USIM_RX_INVALID;
		kal_sprintf(sim_dbg_str,"[SIM_DRV:%d][ERR]RXERR,usim_dcb->ev_status:%x, int_status:%x\n\r",hw_cb->simInterface,usim_dcb->ev_status, int_status);
		DRV_ICC_print_str(sim_dbg_str);
	...
	if(int_status & SIM_STS_TXERR || int_status & SIM_STS_OV)
	{
		...
		usim_set_timeout(0, hw_cb);
		usim_dcb->ev_status = USIM_RX_INVALID;
		kal_sprintf(sim_dbg_str,"[SIM_DRV:%d][ERR]TXERR,usim_dcb->ev_status:%x, int_status:%x\n\r",hw_cb->simInterface,usim_dcb->ev_status, int_status);
		DRV_ICC_print_str(sim_dbg_str);
	}
	if(int_status & SIM_STS_NATR)
	{
		...
		SIM_DisAllIntr();
		usim_dcb->ev_status = USIM_NO_ATR;
		kal_sprintf(sim_dbg_str,"[SIM_DRV:%d][ERR]No ATR\n\r",hw_cb->simInterface);
		DRV_ICC_print_str(sim_dbg_str);
		USIM_SET_EVENT_Multiple(usim_dcb);
	}
...
void usim_hisr2(void)
{
	...
	if(usim_dcb->previous_state == ACTIVATION_STATE || usim_dcb->previous_state == ATR_STATE)
	{
		...
		if(usim_dcb->atr_count > 40)
		{
			...
			USIM_SET_EVENT_Multiple(usim_dcb);
			usim_dcb->atr_count = 0;
			kal_sprintf(sim_dbg_str, "[SIM DRV:%d]SIM2 card send too many ATR data\n\r", hw_cb->simInterface);
	             DRV_ICC_print_str(sim_dbg_str);
			goto end_of_hisr;
		}
	} else
		...
		case (volatile usim_main_state_enum) ACTIVATION_STATE:
			// dbg_print("ACTIVATION_STATE \r\n");
			{
				...
				else
				{
					...
					kal_sprintf(sim_dbg_str, "[SIM_DRV:%d][ERR]Check HW Waveform for HW issue. TS:%x T0:%x usim_dcb->ev_status:%x, int_status:%x\n\r",hw_cb->simInterface,TS, T0, usim_dcb->ev_status, int_status);
					DRV_ICC_print_str(sim_dbg_str);
				}
				USIM_SET_EVENT_Multiple(usim_dcb);
			}
	...
	if(usim_dcb->warm_rst == KAL_FALSE)
	{
		...
		if(SIM_base == hw_cb->mtk_baseAddr)
		{
			...
			if(IRQ_SIM_CODE != hw_cb->mtk_lisrCode)
				ASSERT(0);
			if(usim_hisrid == NULL) {usim_hisrid = kal_init_hisr(USIM_HISR);}
		...
		else if(SIM2_base == hw_cb->mtk_baseAddr)
		{
			...
			if(IRQ_SIM2_CODE != hw_cb->mtk_lisrCode)
				ASSERT(0);
			if(usim2_hisrid == NULL) {usim2_hisrid = kal_init_hisr(USIM2_HISR);}
			if((void *)usim_dcb->gpt_handle == NULL)
				DRV_ICC_GPTI_GetHandle(&usim_dcb->gpt_handle);
#if defined(__SIM_ACTIVATION_V2__)
			if((void *)usim_dcb->gpt_handle_for_SIM_activation == NULL)
...
restart_new_clock:
	...
	*/
	...
	for(retry = 0; retry < ATR_RETRY; retry++)
	{
		...
		{
			dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "[%s,%d] retry %d, ori_ExpectVolt:%d\n\r", __FUNCTION__, __LINE__, retry, ori_ExpectVolt);
			...
			} else
			{
				...
				kal_sprintf(sim_dbg_str, "[SIM_DRV:%d]try SIM activation v2\n\r", hw_cb->simInterface);
				DRV_ICC_print_str(sim_dbg_str);
				...
				kal_sprintf(sim_dbg_str, "[SIM_DRV:%d]try special mode\n\r", hw_cb->simInterface);
				DRV_ICC_print_str(sim_dbg_str);
				if(usim_select_power((usim_power_enum) ori_ExpectVolt, hw_cb) == KAL_FALSE)
				{
					if(usim_dcb->warm_rst == KAL_TRUE)
				...
				{
					  kal_sprintf(sim_dbg_str, "[SIM_DRV:%d] Reset with Abnormal Card\n\r",hw_cb->simInterface);
			                    DRV_ICC_print_str(sim_dbg_str); 
					break;
				}
#endif // #if defined(__ABNORMAL_CARD__)
...
static sim_status L1usim_Cmd(kal_uint8  *txData,kal_uint32  *txSize,kal_uint8  *rxData, kal_uint32  *rxSize, sim_HW_cb *hw_cb)
{
	...
	kal_sprintf(sim_dbg_str, "%s(%d) P3=%d txSize=%d, rxData%s=NULL, *rxSize=%d\n\r",
...
usim_status_enum L1sim_Reset_MTK(sim_power_enum ExpectVolt, sim_power_enum *ResultVolt, kal_bool warm, sim_HW_cb *hw_cb)
{
    ...
    {
        kal_sprintf(sim_dbg_str, "[SIM_DRV:%d]: Skip reset while PollTimerStart!\n\r",hw_cb->simInterface);
	    DRV_ICC_print_str(sim_dbg_str);
        return USIM_NO_INSERT;
    }
#endif
...
/*#ifdef DUAL_MCU_SIM_V1
		kal_sleep_task(KAL_TICKS_500_MSEC);
...
sim_status L1sim_Cmd_MTK(kal_uint8  *txData,kal_uint32  *txSize,kal_uint8  *rxData, kal_uint32  *rxSize, sim_HW_cb *hw_cb)
{
	...
	{
           kal_uint8 rxDataTMP[512]={0};
           kal_uint8 copyLocal=0;
           kal_uint32 rx_buf_len =*rxSize;
           if(usimCard->cmd_case == usim_case_4&&*(txData+1)==0xA4&&rxData!=NULL)
          {
                if(rx_buf_len==0)
                    rx_buf_len=256;
                copyLocal=1;
                *rxSize=512;
                kal_sprintf(sim_dbg_str,"[%s]Reset rxSize to 512,rx_buf_len:%d\n\r",__func__,rx_buf_len);
                DRV_ICC_print_str(sim_dbg_str);
		...
		DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC19, *txSize,*rxSize,result,usimCard->isSW6263,SimCard->cmd_case);
            ...
            if(copyLocal==1) // use a variable since the txData[1] might be modified
            {
