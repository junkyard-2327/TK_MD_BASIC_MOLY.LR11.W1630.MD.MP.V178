...
#if defined(MT6589) || defined(MT6572) || defined(MT6582) || defined(MT6592) || defined(MT6571) || defined(MT6595)
...
#define AP_AFE_SIDDETONE_GAIN              (BASE_MADDR_AUDIO + 0x01EC)
#endif // defined(MT6589) || defined(MT6572) || defined(MT6582) || defined(MT6592) || defined(MT6571) || defined(MT6595)
...
{	
	...
	if(gSidetoneInfo.isActive){
		static kal_uint16 pre_targetValue = 0xFFFF;
    ...
    kal_dev_trace(TRACE_GROUP_SP_DEBUG, SIDETONE_GAIN_INFO, pre_targetValue, targetValue);
  	if((pre_targetValue != targetValue)|| (gSidetoneInfo.isForceSetValue == true)){
       // write to ap hw
       WriteApReg(AP_AFE_SIDDETONE_GAIN, targetValue);	
       pre_targetValue = targetValue;
	   gSidetoneInfo.isForceSetValue = false;
    }
		...
		kal_set_timer(gSidetoneInfo.timer, (kal_timer_func_ptr)(sidetone_hw_UpdateHwStVolume), NULL, AUD_1TICK(4), AUD_1TICK(0));
	}
...
{ 
	...
	if(gSidetoneInfo.isActive){
        ...
        tempTargetValue = (kal_uint32)((kal_uint32)gSidetoneInfo.extStVolume << 2);
...
*/
void SIDETONE_UpdateStart(kal_bool isForce)
{
...
void SIDETONE_Init(void)
{
	...
	// init value sync with AP side int value
	gSidetoneInfo.extStVolume = 0;
    gSidetoneInfo.isForceSetValue = true;
