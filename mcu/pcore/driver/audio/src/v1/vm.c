...
// record syn_word for VM_LOG_DEBUG 
   ...
   *vmBuf++ = (vmGCodecULBufWritePos & 0x0FFF) | 0xA000;//vm.control_1. Bit15 is for 32bit time stamp;
   vmBuf_temp = vmBuf;
...
{	
   ...
   Sal_VM_Frame vmfrm_t;
	...
	SAL_VM_GetFrame3G(&vmfrm_t);
	sd_mode = gEVS_DrvPar.VM_Codec_ID;
	sc_mode = gEVS_DrvPar.VM_Codec_ID;
	...
	I = I & 0x0e;
	vm.control_2 = I << 6 | (sd_mode << 1) | (vm.control_2 & 1);
	if(SPEECH_GOOD_FRAME == gEVS_DecPar.DecPkt.u2BFI)
	{
		vm.control_2 &= 0x7FFF;
	}
	else
  ...
  J =  fma_get_glb_ts() & 0xFFFFFFFF;
	...
	*vmBuf++ = I;
	...
	// record vm control value 	
	vm.control_1 = vm.control_1 | 0x8000;  // Bit15 is for 32bit time stamp
	*vmBuf++ = vm.control_1;
	*vmBuf++ = vm.control_2;

	// record UL data
	*vmBuf++ = gEVS_EncPar.EncPkt.u2BFI;
	*vmBuf++ = gEVS_EncPar.EncPkt.u2Len;
	addr = (volatile uint16 *)EDrv_encoder_get_HBAddr();
	for( I = 0; I < (sc_len - 2); I++ )
		*vmBuf++ = *addr++;
		
	// record DL data 
	*vmBuf++ = gEVS_DecPar.DecPkt.u2BFI;
	*vmBuf++ = gEVS_DecPar.DecPkt.u2Len;	
	addr = (volatile uint16 *)gEVS_DecPar.DecPkt.pucHB;
	...
	addr = vmfrm_t.dbgInfo_addr;  
	*vmBuf = (*addr) & 0xE8;
	*vmBuf = *vmBuf | (SPEECH_BAD_FRAME == gEVS_EncPar.EncPkt.u2BFI) | (gEVS_EncPar.bDtx << 1) | ((SPEECH_BAD_FRAME == gEVS_DecPar.DecPkt.u2BFI) << 2);
	vmBuf++;
	
	// Debug info 1
	*vmBuf = 0;
	*vmBuf = (gEVS_EncPar.u2DSP_sampling_rate >> 4) | ((gEVS_DecPar.u2DSP_sampling_rate >> 4) << 2) | (gEVS_EncPar.u2Max_rate << 4) | (gEVS_EncPar.EncCA.enable << 6);
	*vmBuf = *vmBuf | (gEVS_EncPar.EncCA.rf_fec_indicator << 7) | (gEVS_EncPar.EncCA.rf_fec_offset << 8) | (gEVS_DecPar.DecCA.FrameMode << 11);
	vmBuf++;
...
void vmRecordHisr( void )
{
   ...
   //volatile uint16   *addr;
   uint16   I = 0; //, sc_mode, sd_mode, sc_len, sd_len;
   uint16   *vmBuf;
   // uint32   vmLen = 0;
...
//         kal_trace( TRACE_FUNC,VM_VMRECORDHISR_G_CODEC);
//         ASSERT(0);
//      }else
//#endif
      {
...
//extern kal_bool   SP4G_IsEVSCodecMode();
//void vmFormatter_4g(int16 *vmBuf)
//{
//   if( SP4G_IsGCodecMode() ){//G-serial
//      #if defined(__G_CODEC_SUPPORT__) && defined(__VOLTE_SUPPORT__) 
//      vmFormatter_gseries((uint16 *)vmBuf);
//      #endif //#if defined(__G_CODEC_SUPPORT__) && defined(__VOLTE_SUPPORT__) 
//   }else if( SP4G_IsEVSCodecMode() ){//EVS
//      #if defined(__EVS_CODEC_SUPPORT__) && defined(__VOLTE_SUPPORT__)
...
#if defined(__UMTS_RAT__)// when 3G
...
//         kal_trace( TRACE_FUNC, VM_TCHRECORDHISR_G_CODEC);
//         I =   (uint16)( vmBuf[1] >> 3 )
...
void VMREC_Start(void (*vm_hdlr)(void), uint16 type, bool isVoc)
{
   ...
   if( AM_IsAmInSpeechState() ) {
	...
	//TDD load need to reduce trace
	    if(!gmss_is_tdscdma_mode_activated()){
