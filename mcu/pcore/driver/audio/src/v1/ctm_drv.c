...
{ 
    ...
    savedMask = SaveAndSetIRQMask();    /* Disable interrupt to prevent from race condition */  
...
kal_uint16 TTY_LTE_PSR_UL_GetString(kal_uint8 call_id, kal_uint8 *buf, uint16 *length){
	return  0;
}
...
{ 
    kal_bool isEmpty = false;
    kal_uint32 savedMask;
    
    savedMask = SaveAndSetIRQMask();    /* Disable interrupt to prevent from race condition */  
    isEmpty = RB_EMPTY(ul_tty_queue);
    RestoreIRQMask(savedMask);
...
{ 
    kal_bool isFull = false;
    kal_uint32 savedMask;
    
    savedMask = SaveAndSetIRQMask();    /* Disable interrupt to prevent from race condition */  
    isFull = RB_FULL(ul_tty_queue);
    RestoreIRQMask(savedMask);
...
//      *AFE_VLB_CON |= 0x02;
   ...
   if(VCO_MODE == l1ctm->interface){
      ...
      kal_prompt_trace(MOD_L1SP,"[CTM][SPE] 1, mode = %d",l1ctm->interface);
   	  
   	  SetSPEnhancePath(SpeechEnh_UL); 
      SAL_ENH_Dynamic_Ctrl(0, 9);
      l1sp_setCtmSpeStatus(0);
      kal_prompt_trace(MOD_L1SP,"[CTM][SPE] 0, mode = %d",l1ctm->interface);
   }
   else if(HCO_MODE == l1ctm->interface){
