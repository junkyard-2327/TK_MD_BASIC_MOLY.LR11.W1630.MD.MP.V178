...
{
   kal_int32 index1=0, index2=0, iInByte=0, iOutSample=0, status=0, dwBtEv3HalfBad=0;
   kal_uint32 iOutByte1=0, iOutByte2=0;
   kal_uint8 *pDstBuf;
   while(1)
   {
...
static void BT_SCO_Start(BT_SCO_MODE uMode, kal_bool fIsWideBand)
{
   ...
   btsco.uAudId = L1Audio_GetAudioID();
    if (btsco.uMode == BT_SCO_MODE_LOOPBACK_WITHOUT_CODEC || btsco.uMode == BT_SCO_MODE_LOOPBACK_WITH_CODEC) {
        // kal_prompt_trace(MOD_L1SP, "[OO]disallow sleep, mode = %d, id = %d", btsco.uMode, btsco.uAudId);
        L1Audio_DisallowSleep(btsco.uAudId);
    }
...
#endif
    if (btsco.uMode == BT_SCO_MODE_LOOPBACK_WITHOUT_CODEC || btsco.uMode == BT_SCO_MODE_LOOPBACK_WITH_CODEC) {
        // kal_prompt_trace(MOD_L1SP, "[OO]allow sleep, mode = %d, id = %d", btsco.uMode, btsco.uAudId);
        L1Audio_AllowSleep(btsco.uAudId);
    }
...
#endif
   kal_prompt_trace(MOD_L1SP, "[BT_DBG]BT_SCO_SPEECH_ON");
...
void BT_SCO_SPEECH_OFF()
{
   ...
   kal_prompt_trace(MOD_L1SP, "[BT_DBG]BT_SCO_SPEECH_OFF");
