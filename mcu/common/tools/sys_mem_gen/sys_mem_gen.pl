...
my @symbols=qw/sys_comp_config_tbl 
               ...
               cfg_cc_afm_cb
               cfg_cc_afm_pool_cb
               cfg_cc_afm_MB_overhead
               cfg_cc_afm_pool_stat
               cfg_cc_afm_MB_stat/; 
my @symbols_offset;
my @symbols_size;
...
#CC Ctrl buffer begin
my $g_cc_ctrlbuff_amount = 0;
#CC Ctrl buffer end
...
my ($flag, $sys_mem, $est_mem, $debug_mem, $int_mem, $cc_mem, $cc_dbg_mem, $all_logs) =
...
if ($flag == $ERR_OK)
{
    ...
    if (&output_file($sys_mem, $est_mem, $debug_mem, $int_mem, $cc_mem, $cc_dbg_mem, $all_logs, $outputpath, $platform, $logpath) != 0)
    {
...
sub output_file
{
    ...
    my $cc_mem_size = shift;
    my $cc_dbg_mem_size = shift;
    my $all_logs = shift;
    my $outurl = shift;
    my $platform = shift;
    my $logpath = shift;
 ...
 */
    ...
    #define CC_CtrlBuff_TotalSize_ByGen         ($cc_mem)
    #define CC_CtrlBuff_DBG_MEM_ByGen           ($cc_dbg_mem)
#endif  
__TEMPLATE
    ...
    $all_logs.="sys_mem:".$accurate_sys_mem."\t".
               "estimate sys mem:".$estimate_sys_mem."\t".
               "cc mem:".$cc_mem."\t".
               "debug_mem_size:".$debug_mem_size."\t".
               "int ram size:".$intram_size."\n";
...
## the follwing parameter are from syscomp_config.o
    ...
    my $cc_afm_cb_size;
    my $cc_afm_pool_cb_size;
    my $cc_afm_MB_overhead_size;
    my $cc_afm_pool_stat_size;
    my $cc_afm_MB_stat_size;
    ...
    ($flag, $int_ram_defined,  $debug_buf2_defined,  
        ...
        $cc_afm_cb_size, $cc_afm_pool_cb_size, $cc_afm_MB_overhead_size,
        $cc_afm_pool_stat_size, $cc_afm_MB_stat_size)
...
#  symbols[6] = cfg_cc_ctrl_buff.  NOT system memory
    ($flag, $sys_mem, $dbg_mem, $int_mem, $logs) = 
    &Caculate_cc_control_buffer($syscomp_fh, $symbols_size[6], $symbols_offset[6], 
        $ctr_buff_size,
        $cc_afm_cb_size, $cc_afm_pool_cb_size, $cc_afm_MB_overhead_size,
		$cc_afm_pool_stat_size, $cc_afm_MB_stat_size); 
    if ($flag == 0)
...
#print "fsm memory $new\n";
    ...
    {
        return ($ERR_OK, $sys_mem_size, $estimate_sys_mem, $debug_mem_size, $intram_size, $cc_mem, $cc_dbg_mem, $all_logs); 
    }
    else
    {
        return ($ERR_OK, $total_sys_mem_stack_isolation, 0, $debug_mem_size, $intram_size, $cc_mem, $cc_dbg_mem, $all_logs);
    }
}
...
sub Caculate_cc_control_buffer
{
    my $fh = shift;
    my $sys_size = shift;
    my $sys_offset = shift;
    my $buff_entry_size =  shift;
    my $cc_afm_cb_size = shift;
    my $cc_afm_pool_cb_size = shift;
    my $cc_afm_MB_overhead_size = shift; 
    my $cc_afm_pool_stat_size = shift;
    my $cc_afm_MB_stat_size = shift; 
    ...
    { 
       ...
       ($flag, $cc_mem, $dbg_mem, $int_mem) = 
        &caculate_cc_ctrl_buff_resource($size, $num,
                    $cc_afm_pool_cb_size, $cc_afm_MB_overhead_size,
                    $cc_afm_pool_stat_size, $cc_afm_MB_stat_size);
       if ($flag)
       {
            $all_cc_mem += $cc_mem;
       ...
       $all_logs .= sprintf("cross-core control buffer size(=%6d) num(=%4d)\n", $size, $num);
    }
    $all_logs.="cross-core control buffer consumes sys_mem + dbg_mem(0) = $all_cc_mem + $all_dbg_mem\n";
    
    return (1, $all_cc_mem, $all_dbg_mem, $all_int_mem, $all_logs);
}



sub caculate_cc_ctrl_buff_resource
{
    my $data_size = shift;
    my $num = shift;
    ...
    $cc_mem = ($cc_afm_MB_overhead_size + $data_size) * $num + $cc_afm_pool_cb_size + 32 ;  # 32: cache line size for alignment;
    $debug_mem = $cc_afm_pool_stat_size + $cc_afm_MB_stat_size * $num ;
    return (1, $cc_mem, $debug_mem, 0, undef);
}
