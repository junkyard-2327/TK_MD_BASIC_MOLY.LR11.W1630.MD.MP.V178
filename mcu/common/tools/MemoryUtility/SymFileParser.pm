...
# Constants
#****************************************************************************
my $SYM_FILE_PARSER_VERNO = " u0.08";
                            #  u0.08 , 2017/May/25,Carl,  Use constant instead of using hardcode value
...
{
    my ($strSection) = @_;
    my $strStart = GetLinkerSymbolAddress($strSection, LinkerSymPostfix::Base, LinkerSymPrefix::None);
    my $strEnd = GetLinkerSymbolAddress($strSection, LinkerSymPostfix::Limit, LinkerSymPrefix::None);
    my @SymInfo;
...
sub GetLinkerSymbolAddress
{
    ...
    my $strPrefix = 'Image$$';
    if($nRegionType == LinkerSymPrefix::Load)
    {
        $strPrefix = 'Load$$' ;
        $nLocation = 0;
    }
    elsif($nRegionType == LinkerSymPrefix::None)
    {
        $strPrefix = "";    
    }
