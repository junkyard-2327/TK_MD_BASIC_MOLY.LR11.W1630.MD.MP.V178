...
sub GetChipInsideRAMregion
{
    ...
    {
        if($_ =~ /EMIINIT_CODE|INTSRAM|L2SRAM/)
        {
            $nINTSRAMnum++;
...
sub GetChipInsideRAMsize
{
    ...
    {
        my $strMemory = $_->[0];
        if($strMemory =~ /TCM|ITCM|DTCM|L2SRAM|INTSRAM_MULTIMEDIA/)
        {
            $nINTSRAMnum++;
            my $nMaxSize = hex($_->[2]);
...
sub GetTCMmargin
{
    ...
    my ($ITCMusedsize, $DTCMusedsize, $BTCMusedsize, $L2SRAMusedsize) = (0,0,0,0);
    my %hDSMSectionRecord;
    ...
    foreach my $strINTSRAM (keys %{$TCMregion_href})
    {
        ...
        if( $strINTSRAM =~ /^INTSRAM/)
        {
            if($strINTSRAM =~ /CODE/)
            {
                $ITCMusedsize += $nINTSRAMSize ;
            }
            elsif($strINTSRAM =~ /DATA$|DATA_ZI$/)
            {
                $DTCMusedsize += $nINTSRAMSize;
            }
            else
            {
                $BTCMusedsize += $nINTSRAMSize;
            }
        }
        ...
        # record Dynamic Section Management, only support merge A,B TCM
        if( $strINTSRAM =~ /^DYNAMIC_SECTION_INTSRAM/)
        {
            push @{$hDSMSectionRecord{$nINTSRAMVMA}}, $nINTSRAMSize;
        }
    }
    
    # process Dynamic Section Management, only support merge A,B TCM
    foreach my $nINTSRAMSize (keys %hDSMSectionRecord)
    {
        my $max_size = max @{$hDSMSectionRecord{$nINTSRAMSize}};
        $ITCMusedsize += $max_size;
    }    
