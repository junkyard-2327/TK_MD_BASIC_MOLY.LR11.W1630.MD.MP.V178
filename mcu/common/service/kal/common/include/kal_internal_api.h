...
extern kal_bool   kal_is_mod_id_on_current_core(module_type module_id);
...
#if __PCORE__
#define kal_inline_is_mod_id_on_current_core(module_id)    ( module_id<MOD_ID_CO_PROCESSOR_BASE ? KAL_TRUE : KAL_FALSE )
#else
#define kal_inline_is_mod_id_on_current_core(module_id)    ( module_id>MOD_ID_CO_PROCESSOR_BASE ? KAL_TRUE : KAL_FALSE )
#endif
...
/* under construction !*/
#else
#define kal_enque_msg(ext_msg_q_id, buffer_ptr, msg_size, wait_mode, message_to_head) \
...
#define kal_fatal_error_handler(code1, code2) do { MDM_kal_fatal_error_handler(code1, code2); } while(0)
#endif
void kal_exit(void);
