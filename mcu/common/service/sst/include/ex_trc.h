...
BEGIN_TRACE_MAP(MOD_EX_LOG)
    ...
    TRC_MSG(EX_LOG_TYPE_CC_EXCEPTION, "Exception type: Cross Triggered Exception(%s)")
    ...
    TRC_MSG(EX_LOG_ENVINFO_TIMESTAMP, "exception timestamp: USCNT = %u, Global Timestamp = %u")
    ...
    TRC_MSG(EX_LOG_ENVINFO_STACKDUMP, "    [ADDR:0x%08X]")
    ...
    TRC_MSG(EX_LOG_DIAGINFO_DIAGNOSIS_L2SRAMCORRUPT, "diagnosis: L2SRAM corrupted")
    ...
    TRC_MSG(EX_LOG_DIAGINFO_DIAGNOSIS_ADDRESS, "corrupt address: 0x%08X")
    TRC_MSG(EX_LOG_DIAGINFO_DIAGNOSIS_TIMEING_CHECK, "timing check: %u %u %u %u %u")
    ...
    TRC_MSG(EX_LOG_CCCTRLBUF_SIZE, "cc ctrl buff size = %u")
    TRC_MSG(EX_LOG_CCCTRLBUF_NUM, "# of cc ctrl buff entries = %u")
    ...
    /* cc exception info */
    TRC_MSG(EX_LOG_CC_EX_INFO_ECT, "ect status: 0x%08X")
    TRC_MSG(EX_LOG_CC_EX_INFO_REASON, ">> Exception triggered by %s")
    TRC_MSG(EX_LOG_CC_EX_INFO_MD32, " -- MD32 exception occurs")
    TRC_MSG(EX_LOG_CC_EX_INFO_CS, " -- SONIC exception occurs")
    /* CPU registers */
    ...
    TRC_MSG(EX_LOG_PCMON_PCPAIR_TITLE, "[PC->PC]")
    TRC_MSG(EX_LOG_PCMON_PCPAIR, "(PC)[ADDR:0x%08X] -> (PC)[ADDR:0x%08X]")
    TRC_MSG(EX_LOG_PCMON_PDPAIR_TITLE, "[PC->Data Address]")
    TRC_MSG(EX_LOG_PCMON_PDPAIR, "(PC)[ADDR:0x%08X] -> (DATA ADDR)0x%08X")
    ...
    /* emi mpu violation */
    TRC_MSG(EX_LOG_EMI_MPU_VIO_TITLE, "EMI MPU Info:")
    TRC_MSG(EX_LOG_EMI_MPU_VIO_ADDR, "Violation address = 0x%08X")
    TRC_MSG(EX_LOG_EMI_MPU_VIO_VALUE, "EMI status = 0x%08X")
