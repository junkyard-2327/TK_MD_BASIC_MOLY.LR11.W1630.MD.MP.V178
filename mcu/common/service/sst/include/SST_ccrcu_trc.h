 ...
 ****************************************************************************/
	...
	TRC_MSG(SST_PHASE1_2109_rcuptr, "In rcu_get_free_buff ( ), the address of RCU(0x%X) is not in the non-cacheable memory.")
	TRC_MSG(SST_PHASE1_210A_buffptr, "In rcu_commit_data ( ), the address of RCU block(0x%X) is not in the non-cacheable memory.")
	TRC_MSG(SST_PHASE1_210B_rcuptr, "In rcu_get_read_lock ( ), the address of RCU(0x%X) is not in the non-cacheable memory.")
	TRC_MSG(SST_PHASE1_210C_buffptr, "In rcu_free_read_lock ( ), the address of RCU block(0x%X) is not in the non-cacheable memory.")
	TRC_MSG(SST_PHASE1_210D_rcuptr, "In rcu_create ( ), the address of RCU(0x%X) is not in the share memory.")
	TRC_MSG(SST_PHASE1_210E_buffptr, "The control block of RCU or the control block of RCU block may be corrupted.")
	...
	TRC_MSG(SST_PHASE3_2109_rcuptr_1, "Please convert the RCU address to non-cacheable by dynamic switchable API before")
	TRC_MSG(SST_PHASE3_2109_rcuptr_2, "passing to rcu_get_free_buff().")
	TRC_MSG(SST_PHASE3_210A_buffptr_1, "Please convert the block address to non-cacheable by dynamic switchable API before")
	TRC_MSG(SST_PHASE3_210A_buffptr_2, "passing to rcu_commit_data().")
	TRC_MSG(SST_PHASE3_210B_rcuptr_1, "Please convert the RCU address to non-cacheable by dynamic switchable API before")
	TRC_MSG(SST_PHASE3_210B_rcuptr_2, "passing to rcu_get_read_lock().")
	TRC_MSG(SST_PHASE3_210C_buffptr_1, "Please convert the block address to non-cacheable by dynamic switchable API before")
	TRC_MSG(SST_PHASE3_210C_buffptr_2, "passing to rcu_free_read_lock().")
	TRC_MSG(SST_PHASE3_210D_rcuptr, "Please use the kernel service macro to create RCU memory and pass it correctly.")
	TRC_MSG(SST_PHASE3_210E_buffptr, "Please check the usage of RCU block and do not corrupt the RCU block control block.")

	/* Dump data structure */
	/* RCU */
	TRC_MSG(SST_RCU_CB, "Content of RCU control block as below:")
	TRC_MSG(SST_RCU_CB_SPINLOCK_LOCK, "   rcu_spinlock.lock : %d")
	TRC_MSG(SST_RCU_CB_SPINLOCK_OWNER_MODULE, "   rcu_spinlock.owner_module : %d")
