...
DECLARE_SHARED_VAR(volatile kal_uint32, CCIF_SHARE_MEM_BASE)
DECLARE_SHARED_VAR(volatile kal_uint32, CCIF_SHARE_MEM_SIZE)
 ...
 * Driver Options
 ******************************************************************/
#if !defined(__SSDVT_TEST__) && ( (defined(MT6280) && !defined(__SMART_PHONE_MODEM__)) || defined(MT6290))&& ! defined(__ESL_COSIM_LTE__)
// [Note]
...
/* AP-MD share memory region data Structure */
...
#ifdef __PCORE__
#define P_MCUDOMAIN              (BASE_MADDR_MDPERIMISC +0xF0)
#define P_BUS_AXIDOMAIN          (BASE_MADDR_MDPERIMISC +0x28)
#endif

#define MDL1MCU_GDMA_AXIDOMAIN   (L1_BASE_MADDR_MDL1_CONF +0x220)/*[2:0] MCU and GDMA domain, bit7 for updates should be 1*/
#define MD_L1_BUS_AXIDOMAIN_CON1 (L1_BASE_MADDR_MDL1_BUS_CONFG +0x450)
#define MD_L1_BUS_AXIDOMAIN_CON2 (L1_BASE_MADDR_MDL1_BUS_CONFG +0x454)
...
/*******************************************************************************
 * FUNCTION
 *  SET_TCM_BACKDOR_REGION
 * DESCRIPTION
 *  set the MPU to open the TCM backdor memory region, 
 * PARAMETERS
 ...
 *  N/A
 * NOTE
 *  (1)This API used only in exception flow.
 ******************************************************************************/
#define SET_TCM_BACKDOR_REGION(region_num, start_addr)                           \
do{                                                                             \
    mpu_set_region_primitive(                                                   \
        region_num,                                                             \
        start_addr,                                                             \
        RGN_SIZE_64MB,                                                          \
        (MPU_PRW_URW|MPU_NORMAL_INNC_OUTNC),                                      \
        1                                                                       \
    );                                                                          \
}while(0)
...
#define EMIMPU_VIO_STATUS       (volatile kal_uint32 *)(BASE_MADDR_EMI_BUS_INTERFACE + 0x1F0)
#define EMIMPU_VIO_ADDR         (volatile kal_uint32 *)(BASE_MADDR_EMI_BUS_INTERFACE + 0x1F8)
#else //TK6291
#define EMIMPU_VIO_STATUS       (volatile kal_uint32 *)(BASE_MADDR_EMI + 0x1F0)
#define EMIMPU_VIO_ADDR         (volatile kal_uint32 *)(BASE_MADDR_EMI + 0x1F8)
#endif //if defined(MT6755)

/******************************************************************
 * Export Function Prototypes
 ******************************************************************/
extern MPU_DUMP ARM_MPU_SETTING;
...
extern void ccif_set_share_mem_mpu(kal_uint32 base_addr, kal_uint32 size);
#if defined(__PCORE__) 
extern void ccif_set_share_mem_mpu_cacheable_region(kal_uint32 base_addr, kal_uint32 size);
#endif
extern void system_set_share_mem(SHM_REGION shm_region[]);
extern void SET_ADT_TCM_REGION(kal_uint32 base_addr, kal_uint32 length);
extern void SET_ADT_L2SRAM_REGION(kal_uint32 base_addr, kal_uint32 length);
extern void EMI_MPU_Vio_Get_Info(EMI_MPU_VIO_INFO* emi_mpu_vio_info);
extern void exception_set_mpu(void);
extern void bus_domain_setting(void);
