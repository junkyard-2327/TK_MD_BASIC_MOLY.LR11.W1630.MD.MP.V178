...
#ifdef __L1CORE__
#if defined(__L1CORE_CACHE_PROFILING__)
__attribute__ ((section ("INTSRAM_RW"))) PMU_L1_CACHE_PROFILING_STATUS volatile l1_cache_profiling_en = PMU_L1_CACHE_PROFILING_DISABLE;
#endif
#endif
...
#ifdef __L1CORE__
__attribute__ ((section ("INTSRAM_ROCODE"))) PMU_L1_CACHE_PROFILING_STATUS L1CORE_PMU_CacheProf_Config(PMU_L1_CACHE_PROFILING_SWITCH cache_switch)
{
#if defined(__L1CORE_CACHE_PROFILING__)
    if(l1_cache_profiling_en == PMU_L1_CACHE_PROFILING_ENABLE)
    {
        CR4PMU_StopCounter(PMU_EVENT_COUNTER_2|PMU_EVENT_COUNTER_3);
        CR4PMU_ResetCounter(PMU_EVENT_COUNTER_2);
        CR4PMU_ResetCounter(PMU_EVENT_COUNTER_3);
        if(cache_switch == PMU_L1_ICACHE)
        {
            CR4PMU_SetEvent(PMU_EVENT_COUNTER_2,PMU_EVENT_ICACHE_MISS);
            CR4PMU_SetEvent(PMU_EVENT_COUNTER_3,PMU_EVENT_ICACHE_ACCESS);
        }
        else if(cache_switch)
        {
            CR4PMU_SetEvent(PMU_EVENT_COUNTER_2,PMU_EVENT_DCACHE_MISS);
            CR4PMU_SetEvent(PMU_EVENT_COUNTER_3,PMU_EVENT_DCACHE_ACCESS);
        }
        else
        {
            ASSERT(0);
        }
        CR4PMU_StartCounter(PMU_EVENT_COUNTER_2|PMU_EVENT_COUNTER_3);
        return PMU_L1_CACHE_PROFILING_ENABLE;
    }
#endif
    return PMU_L1_CACHE_PROFILING_DISABLE;
}

__attribute__ ((section ("INTSRAM_ROCODE"))) kal_bool L1CORE_PMU_CacheProf_Enable()
{
#if defined(__L1CORE_CACHE_PROFILING__)
    l1_cache_profiling_en = PMU_L1_CACHE_PROFILING_ENABLE;
    return KAL_TRUE;
#else
    return KAL_FALSE;
#endif
...
__attribute__ ((section ("INTSRAM_ROCODE"))) kal_bool L1CORE_PMU_CacheProf_Disable()
{
#if defined(__L1CORE_CACHE_PROFILING__)
    if(l1_cache_profiling_en == PMU_L1_CACHE_PROFILING_ENABLE)
    {
        l1_cache_profiling_en = PMU_L1_CACHE_PROFILING_DISABLE;
        CR4PMU_StopCounter(PMU_EVENT_COUNTER_2|PMU_EVENT_COUNTER_3);
        CR4PMU_ResetCounter(PMU_EVENT_COUNTER_2);
        CR4PMU_ResetCounter(PMU_EVENT_COUNTER_3);
    }
...
__attribute__ ((section ("INTSRAM_ROCODE"))) cache_profiling L1CORE_PMU_CacheProf_GetCounter()
{
    cache_profiling prof = {0,0,PMU_L1_CACHE_PROFILING_DISABLE};
#if defined(__L1CORE_CACHE_PROFILING__)
    if(l1_cache_profiling_en == PMU_L1_CACHE_PROFILING_ENABLE)
    {
        prof.miss_count = CR4PMU_ReadCounter(PMU_EVENT_COUNTER_2);
        prof.access_count = CR4PMU_ReadCounter(PMU_EVENT_COUNTER_3);
        prof.status = PMU_L1_CACHE_PROFILING_ENABLE;
    }
#endif
    return prof;
}
