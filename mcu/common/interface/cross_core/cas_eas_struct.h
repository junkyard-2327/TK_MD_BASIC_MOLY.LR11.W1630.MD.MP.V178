 ...
 *
 * 09 01 2016 otto.liu
 * [MOLY00200003] [MT6291]C2K supported band list, interface between MD3 and MD1
 * 	
 * 	C2K supported band list interface
 *
 * 02 03 2016 doug.shih
 * [MOLY00150211] [VzW][VZ_REQ_LTEMMO_38695] LTE Scanning Enhancement
 * New MD1/MD3 cross-core interface (OA domain).
 *
 ...
 * 	.
 *
 * 01 06 2016 shin.chen
 * [MOLY00156136] [LR11_TRUNK] cross-core interface for C2K-LTE time sync with SIB8
 * .
 *
 * 10 27 2015 yenchih.yang
...
typedef struct
{
    ...
    kal_uint8                  band_num;     /* indicate the number of measurement bands, range: 0..MAX_NUM_MEAS_C2K_BAND_INC
                                                if band_num=0, CAS should stop the measurement specified in the 'meas_type' */
    cas_meas_band_info_struct  band_list[MAX_NUM_MEAS_C2K_BAND_INC];  /* indicate the measurement information */
                                              ...
                                              * note: due to CC ILM size limit (4096 bytes), MEAS_IND's band list remain 2 */
...
typedef struct
{
    ...
    kal_uint8                  freq_num;                          /* indicate the number of measurement frequencies, range: 0..MAX_NUM_MEAS_C2K_FREQ */
    cas_meas_freq_rslt_struct  freq_list[MAX_NUM_MEAS_C2K_FREQ];  /* indicate the measurement information */
...
typedef struct
{
    ...
    kal_bool                   is_hps_meas;                       /* When CAS receives measurement result from CL1 for HP_MODE, this field should set as TRUE. 
                                                                     ...
                                                                     set as FALSE. */                                                                 
    kal_uint8                  band_num;                          /* indicate the number of measurement bands, only 1 is allowed */
    cas_meas_band_rslt_struct  band_list[MAX_NUM_MEAS_C2K_BAND];  /* indicate the band measurement result */
                                                                  /* due to CC ILM size limit (4096 bytes)
                                                                   * even max band number increase to 5, MEAS_IND's band list still remain 2
                                                                   * C2K will send one MEAS_IND to ERRC for each C2K band
                                                                   * use "band" in "cas_meas_band_rslt_struct" to indicate which band is measured */
#ifdef UNIT_TEST
...
typedef struct
{
    LOCAL_PARA_HDR
    kal_uint8 dummy;                       /* to prevent compiling error */
    kal_uint32 c2k_supported_band_list;
...
/* CLC_ERRC_EVDO_PS_CONNECTION_STATUS_IND */
typedef struct
{
    LOCAL_PARA_HDR
