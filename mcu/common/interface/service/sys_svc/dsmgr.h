...
/* If you want to specify a section to be loaded by GMDA or CPU, please add 
    DYNAMIC_SECTION_LOAD_BY_GMDA(section_id) or DYNAMIC_SECTION_LOAD_BY_CPU(section_id) in below, respectively.
*/
        
    #if defined(__PCORE__)
        DYNAMIC_SECTION_LOAD_BY_CPU(DYNAMIC_SECTION_INTSRAM_UBIN_W)
        DYNAMIC_SECTION_LOAD_BY_CPU(DYNAMIC_SECTION_INTSRAM_UBIN_T)
    #elif defined(__L1CORE__)
        DYNAMIC_SECTION_LOAD_BY_CPU(DYNAMIC_SECTION_INTSRAM_UBIN_W)
        DYNAMIC_SECTION_LOAD_BY_CPU(DYNAMIC_SECTION_INTSRAM_UBIN_T)
        DYNAMIC_SECTION_LOAD_BY_GMDA(DYNAMIC_SECTION_INTSRAM_ADT_W)
        DYNAMIC_SECTION_LOAD_BY_GMDA(DYNAMIC_SECTION_INTSRAM_ADT_T)
        DYNAMIC_SECTION_LOAD_BY_GMDA(DYNAMIC_SECTION_INTSRAM_ADT_L)
        DYNAMIC_SECTION_LOAD_BY_CPU(DYNAMIC_SECTION_L2SRAM_UBIN_W)
        DYNAMIC_SECTION_LOAD_BY_CPU(DYNAMIC_SECTION_L2SRAM_UBIN_T)
    #endif

#elif defined(__DSM_SPACE_CONFIGURATION__)

/* Configuration of the reserved space size for uploaded data. unit: byte
*/

    #if defined(__PCORE__)
        DYNAMIC_SECTION_RESERVE_SPACE(DYNAMIC_SECTION_INTSRAM_UBIN_W, 0)
        DYNAMIC_SECTION_RESERVE_SPACE(DYNAMIC_SECTION_INTSRAM_UBIN_T, 0)
    #elif defined(__L1CORE__)
        DYNAMIC_SECTION_RESERVE_SPACE(DYNAMIC_SECTION_INTSRAM_UBIN_W, ALIGNTOCACHELINE(33*1024))
        DYNAMIC_SECTION_RESERVE_SPACE(DYNAMIC_SECTION_INTSRAM_UBIN_T, ALIGNTOCACHELINE(41*1024))
        DYNAMIC_SECTION_RESERVE_SPACE(DYNAMIC_SECTION_INTSRAM_ADT_W,  ALIGNTOCACHELINE(0))
        DYNAMIC_SECTION_RESERVE_SPACE(DYNAMIC_SECTION_INTSRAM_ADT_T,  ALIGNTOCACHELINE(0))
        DYNAMIC_SECTION_RESERVE_SPACE(DYNAMIC_SECTION_INTSRAM_ADT_L,  ALIGNTOCACHELINE(0))
        DYNAMIC_SECTION_RESERVE_SPACE(DYNAMIC_SECTION_L2SRAM_UBIN_W,  ALIGNTOCACHELINE(0))
        DYNAMIC_SECTION_RESERVE_SPACE(DYNAMIC_SECTION_L2SRAM_UBIN_T,  ALIGNTOCACHELINE(0))
    #endif

#else /* !efined(__DSM_CONFIGURATION__) && !defined(__DSM_SPACE_CONFIGURATION__) */


#if defined(__PCORE__)

    typedef enum {
        DYNAMIC_SECTION_INTSRAM_UBIN_W = 0,
        DYNAMIC_SECTION_INTSRAM_UBIN_T,
        /* insert id before this line*/
        DYNAMIC_SECTION_NUM     /*2 */
    } dsm_section_id;

    /* make sure to define TOTAL_DSM_SECTION as valuse of DYNAMIC_SECTION_NUM */
    #define TOTAL_DSM_SECTION (2)
    ...
    typedef enum {
        DYNAMIC_SECTION_INTSRAM_UBIN_W = 0,
        DYNAMIC_SECTION_INTSRAM_UBIN_T,
        DYNAMIC_SECTION_INTSRAM_ADT_W,
        DYNAMIC_SECTION_INTSRAM_ADT_T,
        DYNAMIC_SECTION_INTSRAM_ADT_L,
        DYNAMIC_SECTION_L2SRAM_UBIN_W,
        DYNAMIC_SECTION_L2SRAM_UBIN_T,
        /* insert id before this line*/
        DYNAMIC_SECTION_NUM     /* 7 */
    } dsm_section_id;

    /* make sure to define TOTAL_DSM_SECTION as valuse of DYNAMIC_SECTION_NUM */
    #define TOTAL_DSM_SECTION (7)

#endif



#if defined(__MTK_TARGET__) && defined(__L1CORE__)

#define LOAD_DYNAMIC_SECTION_INTSRAM_UBIN_W         DSM_Load(DYNAMIC_SECTION_INTSRAM_UBIN_W);
#define LOAD_DYNAMIC_SECTION_INTSRAM_UBIN_T         DSM_Load(DYNAMIC_SECTION_INTSRAM_UBIN_T);
#define LOAD_DYNAMIC_SECTION_INTSRAM_ADT_W          DSM_Load(DYNAMIC_SECTION_INTSRAM_ADT_W);
#define LOAD_DYNAMIC_SECTION_INTSRAM_ADT_T          DSM_Load(DYNAMIC_SECTION_INTSRAM_ADT_T);
#define LOAD_DYNAMIC_SECTION_INTSRAM_ADT_L          DSM_Load(DYNAMIC_SECTION_INTSRAM_ADT_L);

#define UNLOAD_DYNAMIC_SECTION_INTSRAM_UBIN_W       DSM_Unload(DYNAMIC_SECTION_INTSRAM_UBIN_W);
#define UNLOAD_DYNAMIC_SECTION_INTSRAM_UBIN_T       DSM_Unload(DYNAMIC_SECTION_INTSRAM_UBIN_T);
#define UNLOAD_DYNAMIC_SECTION_INTSRAM_ADT_W        DSM_Unload(DYNAMIC_SECTION_INTSRAM_ADT_W);
#define UNLOAD_DYNAMIC_SECTION_INTSRAM_ADT_T        DSM_Unload(DYNAMIC_SECTION_INTSRAM_ADT_T);
#define UNLOAD_DYNAMIC_SECTION_INTSRAM_ADT_L        DSM_Unload(DYNAMIC_SECTION_INTSRAM_ADT_L);
...
typedef enum {
    ...
    DSM_ACTION_LOAD_END_INIT,
    DSM_ACTION_LOAD_END_RESTORE,
    DSM_ACTION_LOAD_GDMA_LOAD_END,
    DSM_ACTION_LOAD_CANCEL_GDMA_LOAD,
    DSM_ACTION_LOAD_CONFIG_GDMA_LOAD_RO,
    DSM_ACTION_LOAD_CONFIG_GDMA_LOAD_RWZI,
    DSM_ACTION_LOAD_CONFIG_GDMA_LOAD_RORW,
    DSM_ACTION_LOAD_CONFIG_GDMA_LOAD_ZI,
    DSM_ACTION_UNLOAD_START,
    DSM_ACTION_UNLOAD_END_UNLOAD_NOTHING,
...
typedef enum {
    ...
    DSM_UNLOAD_NOTHING,     /* the section has been load before */
} dsm_status;
 ...
 *  N/A
 * NOTE
 *  Fatal error is happened if the specified section is not load by GMDA.
 * SEE ALSO
...
void DSM_ActionRecordInfo(kal_uint32 index, dsm_section_id *dsm_id, kal_uint32 *dsm_tick, DSM_ACTION *dsm_action);
kal_uint32 DSM_IsLocatedInDSMSection(kal_uint32 query_addr);
