...
#define MMRF_CUSTOM_SHARED_DATA_SET_TO_CACHE(ptr, size)   dynamic_switch_cacheable_region(&(ptr), MMRF_ALIGNTOCACHELINE(size), PAGE_CACHEABLE )
#define MMRF_CUSTOM_SHARED_DATA_SET_TO_NOCACHE(ptr, size) dynamic_switch_cacheable_region(&(ptr), MMRF_ALIGNTOCACHELINE(size), PAGE_NO_CACHE )

#define MMRF_CUSTOM_SHARED_DATA_CACHE_FLUSH(ptr, size)        \
{ \
   dynamic_switch_cacheable_region( &(ptr), MMRF_ALIGNTOCACHELINE(size), PAGE_NO_CACHE ); \
   dynamic_switch_cacheable_region( &(ptr), MMRF_ALIGNTOCACHELINE(size), PAGE_CACHEABLE ); \
}
#else
 ...
 *******************************************************************************/
#if defined(__PCORE__)
#define GS_QUEUE_MMRF                       MOD_MMRF_PCORE
#else
 ...
 *******************************************************************************/
   #if defined(__PCORE__)
#define MOD_ID_MMRF                         MOD_MMRF_PCORE
   #else
...
static INLINE kal_uint8* MMRF_GET_PEERBUF(void *_pPeerBuff)
{
   kal_uint16  dummy_length;
   kal_uint8   *pPduData;
   peer_buff_struct *pPeerBuff;

   pPeerBuff = (peer_buff_struct *)_pPeerBuff;

   pPduData = (kal_uint8 *)get_peer_buff_pdu((peer_buff_struct *)pPeerBuff, &dummy_length);
...
static INLINE void MMRF_SEND_MSG(module_type src_mod, module_type dest_mod, sap_type sap_id, msg_type msg_id, local_para_struct *local_para, peer_buff_struct *peer_buf)
{
   ...
   //For simulation, since ul1 do not have sub module, we treate different module as different task temporaly
   //if (mod_task_g[src_mod] == mod_task_g[dest_mod])
   if ( src_mod == dest_mod )
   {  
       gs_send_int_message(queue_id, ilm_ptr, KAL_TRUE);
   }
   else
   {
       gs_send_message(queue_id, ilm_ptr, KAL_TRUE);
   }
}
...
static INLINE kal_uint8* MMRF_ALLOC_PEERBUF(kal_uint32 size)
{
   ...
   if( size )
      pPduData = (kal_uint8 *)malloc(size);
   else
      pPduData = (kal_uint8 *)malloc(1);
...
static INLINE kal_uint8* MMRF_GET_PEERBUF(void *pPeerBuff)
{
